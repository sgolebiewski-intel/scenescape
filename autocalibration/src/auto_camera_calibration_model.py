...
class CalibrationScene:
    ...
    def __init__(self, uid, name):
        ...
        self.mesh_scale = [
            self.DEFAULTS["scale_x"],
            self.DEFAULTS["scale_y"],
    ...
    def deserialize(cls, data):
        ...
        scene.map = (
            "/workspace/media/" + data["map"].split("/")[-1]
            if "map" in data
            else scene.DEFAULTS["map"]
        )
        ...
        scene.map_processed = (
            get_datetime_from_string(data["map_processed"])
            if "map_processed" in data
            else scene.DEFAULTS["map_processed"]
        )
        scene.camera_calibration = data.get(
            "camera_calibration", scene.DEFAULTS["camera_calibration"]
        )
        scene.polycam_data = (
            "/workspace/media/" + data["polycam_data"].split("/")[-1]
            if "polycam_data" in data
            else scene.DEFAULTS["polycam_data"]
        )
        scene.number_of_localizations = data.get(
            "number_of_localizations", scene.DEFAULTS["number_of_localizations"]
        )
        ...
        scene.minimum_number_of_matches = data.get(
            "minimum_number_of_matches", scene.DEFAULTS["minimum_number_of_matches"]
        )
