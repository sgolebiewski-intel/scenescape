...
class CalibrationScene:
    # FIXME: should be defined in common location accessible to both models
    # and camcalibration
    DEFAULTS = {
        'rotation_x': 0.0,
        'rotation_y': 0.0,
    ...
    def __init__(self, uid, name):
        ...
        self.name = name
        self.mesh_rotation = [
            self.DEFAULTS['rotation_x'],
            self.DEFAULTS['rotation_y'],
            self.DEFAULTS['rotation_z']]
        self.mesh_translation = [
            self.DEFAULTS['translation_x'],
            self.DEFAULTS['translation_y'],
    ...
    def deserialize(cls, data):
        ...
        scene.map = "/workspace/media/" + \
            data['map'].split('/')[-1] if 'map' in data else scene.DEFAULTS['map']
        scene.scale = data.get('scale', scene.DEFAULTS['scale'])
        scene.mesh_rotation = data.get('mesh_rotation', scene.mesh_rotation)
        scene.mesh_translation = data.get(
            'mesh_translation', scene.mesh_translation)
        scene.mesh_scale = data.get('mesh_scale', scene.mesh_scale)
        scene.apriltag_size = data.get(
            'apriltag_size', scene.DEFAULTS['apriltag_size'])
        scene.map_processed = get_datetime_from_string(
            data['map_processed']) if 'map_processed' in data else scene.DEFAULTS['map_processed']
        scene.camera_calibration = data.get(
            'camera_calibration',
            scene.DEFAULTS['camera_calibration'])
        scene.polycam_data = "/workspace/media/" + data['polycam_data'].split(
            '/')[-1] if 'polycam_data' in data else scene.DEFAULTS['polycam_data']
        scene.number_of_localizations = data.get(
            'number_of_localizations',
            scene.DEFAULTS['number_of_localizations'])
        scene.global_feature = data.get(
            'global_feature', scene.DEFAULTS['global_feature'])
        scene.local_feature = data.get(
            'local_feature', scene.DEFAULTS['local_feature'])
        scene.matcher = data.get('matcher', scene.DEFAULTS['matcher'])
        scene.minimum_number_of_matches = data.get(
            'minimum_number_of_matches',
            scene.DEFAULTS['minimum_number_of_matches'])
        scene.inlier_threshold = data.get(
            'inlier_threshold', scene.DEFAULTS['inlier_threshold'])
...
class CameraCalibrationModel():
    ...
    def sceneWithID(self, scene_id):
        ...
        if not response:
            log.error(
                f"Failed to get responses for scene {scene_id}, error code: ",
                response.statusCode)
            return
    ...
    def sceneCameraWithID(self, camera_id):
        ...
        if not response:
            log.error(
                f"Failed to get responses for camera {camera_id}, error code: ",
                response.statusCode)
            return
        else:
    ...
    def allScenes(self):
        ...
        if 'results' not in response:
            log.error(
                f"Failed to get responses for all scenes, error code: ",
                response.statusCode)
            return
    ...
    def updateMapProcessed(self, scene_id, map_processed):
        response = self.rest.updateScene(
            scene_id, {'map_processed': map_processed})
        if not response:
            log.error(
                f"Failed to update map processed for scene {scene_id}, error code: ",
                response.statusCode)
            return
        return response
    ...
    def calibrationMarkersWithSceneID(self, scene_id):
        ...
        if not response:
            log.error(
                f"Failed to get responses for calibration markers with scene {scene_id}, error code: ",
                response.statusCode)
            return
        return response
    ...
    def deleteCalibrationMarkersForScene(self, scene_id):
        ...
        if not response:
            log.error(
                f"Failed to delete calibration markers for scene {scene_id}, error code: ",
                response.statusCode)
            return
        found = []
