...
from atag_camera_calibration import (
...
class ApriltagCameraCalibrationController(CameraCalibrationController):
    ...
    def processSceneForCalibration(self, sceneobj, map_update=False):
        ...
        if sceneobj.id not in self.cam_calib_objs or map_update:
            try:
                self.cam_calib_objs[sceneobj.id] = CameraCalibrationApriltag(
                    ...
                    sceneobj.scale,
                    sceneobj.name,
                    tag_size=sceneobj.apriltag_size,
                )
        ...
        if len(self.cam_calib_objs[sceneobj.id].result_data_3d) < MIN_APRILTAG_COUNT:
            response_dict["status"] = (
                "Cannot auto calibrate. Check scene to ensure there "
    ...
    def generateCalibration(self, sceneobj, msg):
        ...
        try:
            ...
            if (
                cur_cam_calib_obj.result_data_3d is None
            ...
            if (
                ...
                cam_pose = CameraPose(camera_pose, intrinsic_matrix_2d)
                ...
                log.info(
                    (
                        "Point correspondences calculated for calibration UI for camera"
                        f" {
                        percebro_cam_data['id']}"
                    )
                )
                ...
                points_3d = [
                    np.dot(self.scene_pose_mat, np.append(point, 1))[:3].tolist()
                ...
                if (
                    percebro_cam_data["id"] not in self.frame_count
                    or self.frame_count[percebro_cam_data["id"]] < MAX_WAIT_FRAME_COUNT
                ):
