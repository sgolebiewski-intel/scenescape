...
class ApriltagCameraCalibrationController(CameraCalibrationController):
    ...
    def processSceneForCalibration(self, sceneobj, map_update=False):
        ...
        log.info(
            "processing apriltags scene for calibration for ",
            sceneobj.name)
        ...
        if sceneobj.id not in self.cam_calib_objs or map_update:
            try:
                self.cam_calib_objs[sceneobj.id] = CameraCalibrationApriltag(
                    sceneobj.map, sceneobj.scale, sceneobj.name, tag_size=sceneobj.apriltag_size)
        ...
        if sceneobj.map_processed is None or map_update:
            try:
                with self.cam_calib_objs[sceneobj.id].cam_calib_lock:
                    self.cam_calib_objs[sceneobj.id].identifyApriltagsInScene(
                        TILE_SIZE, TILE_SIZE, DEFAULT_ROTATION_MATRIX)
                    if self.cam_calib_objs[sceneobj.id].result_data_3d is not None:
                        self.saveToDatabase(
                            sceneobj, self.cam_calib_objs[sceneobj.id].result_data_3d)
                    log.info(
                        "Apriltag center points in 3D identified and saved to database.")
            except FileNotFoundError:
                response_dict['status'] = "Error: Glb file not found"
                return response_dict

        apriltags_from_db = []
        response = self.calibration_data_interface.calibrationMarkersWithSceneID(
            sceneobj.id)
        ...
        if len(
                self.cam_calib_objs[sceneobj.id].result_data_3d) < MIN_APRILTAG_COUNT:
    ...
    def resetScene(self, scene):
        ...
        self.calibration_data_interface.deleteCalibrationMarkersForScene(
            scene.id)
        return
    ...
    def isMapUpdated(self, sceneobj):
        ...
        if not sceneobj.map:
            ...
            return (
                sceneobj.map_processed is None) or (
                self.isMapProcessed(sceneobj))
        ...
        if apriltags and len(atag_points_3d) < len(apriltags):
            ...
            for key, value in atag_points_3d.items():
                post_data = {
                    ...
                    'dims': value,
                    'scene': scene.id}
                self.calibration_data_interface.updateOrCreateCalibrationMarker(
                    scene.id, post_data)
    ...
    def generateCalibration(self, sceneobj, msg):
        ...
        try:
            ...
            if (cur_cam_calib_obj.result_data_3d is None or len(
                    cur_cam_calib_obj.result_data_3d) < MIN_APRILTAG_COUNT):
                raise TypeError((
            ...
            log.info(
                f"Camera pose computed for camera {
                    percebro_cam_data['id']}")

            if (camera_pose is not None and len(
                    cur_cam_calib_obj.apriltags_2d_data) >= MIN_APRILTAG_COUNT):
                ...
                log.info(
                    ("Point correspondences calculated for calibration UI for camera" f" {
                        percebro_cam_data['id']}"))
                ...
                quat = Rotation.from_matrix(
                    cam_to_world_y_down[0:3, 0:3]).as_quat()
                trans = np.ravel(cam_to_world_y_down[0:3, 3:4].flatten())

                # Apply scene pose to 3d calibration points.
                points_3d = [np.dot(self.scene_pose_mat, np.append(point, 1))[
                    :3].tolist() for point in points_3d]
                ...
                if (percebro_cam_data['id'] not in self.frame_count or
                    ...
                    publish_topic = PubSub.formatTopic(
                        PubSub.CMD_CAMERA, camera_id=percebro_cam_data['id'])
                    return {
                        'publish_topic': publish_topic,
                        'publish_data': 'localize'}
                else:
                    raise TypeError((
