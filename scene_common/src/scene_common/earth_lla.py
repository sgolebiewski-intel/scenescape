...
def calculateHeading(trs_mat, map_pt, velocity):
    ...
    y = math.cos(lat_a) * math.sin(lat_b) - math.sin(lat_a) * math.cos(lat_b) * math.cos(
        long_diff
    )
...
def calculateTRSLocal2LLAFromSurfacePoints(
    map_xyz_pts, lla_pts, z_shift: float = DEFAULT_Z_SHIFT_METERS
) -> np.ndarray:
    ...
    lla_pts = np.vstack(
        [
            lla_pts,
            np.column_stack([lla_pts[:, 0], lla_pts[:, 1], lla_pts[:, 2] + z_shift]),
        ]
    )
...
def calculateTRSLocal2LLAFromImageMap(
    ...
    lla_pts,
    z_shift: float = DEFAULT_Z_SHIFT_METERS,
) -> np.ndarray:
    """! Calculates a transformation matrix from local Cartesian coordinates
    ...
    @returns    numpy.ndarray    Transformation matrix in TRS format
    """
    map_pts = (1 / pixels_per_meter) * np.array(
        [[0, 0, 0], [resx, 0, 0], [resx, resy, 0], [0, resy, 0]]
    )
