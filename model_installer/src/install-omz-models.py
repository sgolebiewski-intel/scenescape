...
MODEL_DIR = os.environ.get(
    "MODEL_DIR") if "MODEL_DIR" in os.environ else "/workspace/models"
...
def _build_argparser():
    ...
    parser.add_argument(
        "--default",
        action="store_true",
        help="Download default models")
    parser.add_argument(
        "--ocr",
        action="store_true",
        help="Download OCR models")
    parser.add_argument(
        "--all",
        action="store_true",
    ...
    parser.add_argument(
        "--precisions",
        help="Donwload models with specific precisions."
...
def _find_existing_config(ovms_config):
    existing_config = {config["config"]["name"]
                       for config in ovms_config["model_config_list"]}
    ...
    for model in models:
        ...
        if not os.path.isdir(os.path.join(MODEL_DIR, model_subdir, model)):
            ...
            if not os.path.isdir(source):
                _convert_model(model, precisions, model_subdir,
                               converter_path="omz_converter")
