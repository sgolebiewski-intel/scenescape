# Intel® Edge Spatial Intelligence on Kubernetes

Intel® Edge Spatial Intelligence Kubernetes helm chart

## Overview

This folder contains the helm chart to run Intel® Edge Spatial Intelligence on Kubernetes.

This readme goes through a minimal setup for running this on your local development machine with only 3 extra binaries needed. The default Makefile target starts a Kubernetes cluster in a Docker container using kind, then deploys Intel® Edge Spatial Intelligence to that cluster using helm.
...
The easiest way to install Intel® Edge Spatial Intelligence on Kubernetes is with a single command:
...
- Deploy Intel® Edge Spatial Intelligence to the cluster using Helm
...
If you already have a Kubernetes cluster and want to deploy Edge Spatial Intelligence without the Makefile or `deploy.sh`, you can use the Helm chart directly.
...
If you're deploying Edge Spatial Intelligence in an environment that requires proxy access, set these environment variables before running make commands:
...
The proxy settings will be automatically detected and passed to all Edge Spatial Intelligence containers as environment variables.
...
To deploy Edge Spatial Intelligence in validation/testing mode:
...
2. Build Intel® Edge Spatial Intelligence init-images and edge spatial intelligence images, then push everything to the local registry.
   ```console
   $ make -C kubernetes build-all
   ```
3. Install the Intel® Edge Spatial Intelligence release with helm.
   ```console
   $ make -C kubernetes install
   ```
4. Verify that Intel® Edge Spatial Intelligence is running.
   ```console
   kubectl get pods -n scenescape -w
   # alternative TUI
   k9s
   ```
5. Uninstall the Intel® Edge Spatial Intelligence release.
   ```console
...
- **WARNING: Intel® Edge Spatial Intelligence data isn't persisted, uninstalling the release will lead to data loss.**
...
- How do I verify that everything is working properly?
  Run `k9s` and check that the Intel® Edge Spatial Intelligence pods are `Ready` and in the `Running` status. If they're stuck in an error status, refer to the steps in Troubleshooting.
...
- If you can't access the Intel® Edge Spatial Intelligence webUI, make sure Intel® Edge Spatial Intelligence on Docker isn't running.
