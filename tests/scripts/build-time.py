...
def main():
    parser = argparse.ArgumentParser(
        description="Clean environment and measure build time."
    )
    parser.add_argument(
        "--time-limit", type=int, required=True, help="Time limit in seconds"
    )
    parser.add_argument(
        "--clean-cmd", default="make clean-all", help="Clean command to run before build"
    )
    parser.add_argument(
        "--build-cmd", default="make build-all", help="Build command to measure"
    )
    ...
    if duration < args.time_limit:
        ...
        print(
            f"Build took too long: {duration:.2f} seconds (limit is {args.time_limit} seconds)"
        )
