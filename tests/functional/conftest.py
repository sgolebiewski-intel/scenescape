...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--password", required=True, help="password to log into REST server"
    )
    parser.addoption(
        "--auth",
    ...
    parser.addoption(
        "--broker_port", default="1883", type=int, help="Port of MQTT broker"
    )
    parser.addoption(
        "--weburl",
    ...
    parser.addoption(
        "--scene_name", default="Demo", help="name of scene to test against"
    )
...
def pytest_configure(config):
    ...
    config.option.htmlpath = (
        os.getcwd() + "/tests/functional/reports/test_reports/" + file_name + ".html"
    )
