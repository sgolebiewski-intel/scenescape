...
class APIStrings(FunctionalTest):
    ...
    def runApiStrings(self):
        ...
        assert res.errors['non_field_errors'] == [
        ...
        res = self.rest.createTripwire(
            {"name": random_string, "scene": self.sceneID})
        ...
        assert res.errors['name'] == [
            'Ensure this field has no more than 200 characters.']
        res = self.rest.createSensor(
            {"name": random_string, "scene": self.sceneID})
        print(res.errors['name'])
        assert res.errors['name'] == [
            'Ensure this field has no more than 200 characters.']
        res = self.rest.createCamera(
            {"name": random_string, "scene": self.sceneID})
        print(res.errors['name'])
        assert res.errors['name'] == [
            'Ensure this field has no more than 200 characters.']
        ...
        assert res.errors['name'] == [
            'Ensure this field has no more than 200 characters.']
        res = self.rest.createSensor(
            {"sensor_id": random_string, "scene": self.sceneID})
        print(res.errors['sensor_id'])
        assert res.errors['sensor_id'] == [
