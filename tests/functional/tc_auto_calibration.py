...
class AutoCalibration(FunctionalTest):
    def __init__(self, testName, request, recordXMLAttribute):
        ...
        self.initial_cam_points = [[119.0, 622.0], [
            257.0, 561.0], [978.0, 580.0], [628.0, 312.0]]
        self.initial_map_points = [[1.685, 2.533], [
            2.188, 2.578], [4.449, 1.412], [3.94, 3.228]]
    ...
    def runMqttPrepare(self):
        topic_auto_calib_status = PubSub.formatTopic(
            PubSub.SYS_AUTOCALIB_STATUS)
        topic_data_autocalib_cam_pose = PubSub.formatTopic(
            PubSub.DATA_AUTOCALIB_CAM_POSE, camera_id=self.test_camera)
        self.pubsub.addCallback(
            topic_data_autocalib_cam_pose,
            self.collectPoseResults)
        self.pubsub.addCallback(
            topic_auto_calib_status,
            self.autoCalibrationStatus)
    ...
    def runAutoCalibration(self):
        ...
        try:
            ...
            if self.updated_map_points is not None and self.updated_cam_points is not None:
