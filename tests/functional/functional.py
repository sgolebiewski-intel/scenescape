...
class FunctionalTest(Diagnostic):
    ...
    def sceneControllerReady(
        self, waitTopic, publishTopic, timeout, beginEpoch, interval, detection
    ):
    ...
    def sceneScapeReady(self, max_attempts, controller_wait):
        ...
        waitTopic = PubSub.formatTopic(
            PubSub.DATA_SCENE, scene_id=self.params["scene_id"], thing_type="person"
        )
        ...
        while attempts < max_attempts:
            ...
            ready = self.sceneControllerReady(
                waitTopic, publishTopic, controller_wait, begin, 1 / frameRate, objData
            )
            if ready:
                break
        else:
