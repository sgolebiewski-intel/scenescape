...
def build_argparser():
    ...
    sub_parser = subparsers.add_parser(
        "update-junit",
        help="Parse output from `make --trace`"
        " and update junit files")
    sub_parser.add_argument(
        "log", help="Path to log file with `make -C tests` output")
    sub_parser.add_argument(
        "--ignore-skipped",
        action="store_true",
    ...
    sub_parser.add_argument(
        "commit",
        nargs="?",
        help="get list of broken tests added by commit")
    sub_parser.add_argument(
        "--all",
        action="store_true",
...
def update_junit(args):
    ...
    print("   executed:", len(
        [x for x in allTests if allTests[x].status != TestResult.SKIPPED]))
    print("     failed:", len(
        [x for x in allTests if allTests[x].status == TestResult.FAILED]))
    print("    skipped:", len(
        [x for x in allTests if allTests[x].status == TestResult.SKIPPED]))
...
def newly_broken(args):
    ...
    for commit in commits[1:]:
        ...
        if (not args.broken_only and cur_broken !=
                prev_broken) or cur_broken - prev_broken:
            break
