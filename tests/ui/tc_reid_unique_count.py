    ...
    while time.time() - start_time < TEST_WAIT_TIME:
        ...
        for scene in detection_count:
            if detection_count[scene]["current"] <= detection_count[scene]["maximum"]:
                log.info(
                    f"-> Detections for {scene} of: {detection_count[scene]['current']} (max: {detection_count[scene]['maximum']})"
                )
            else:
                log.error(
                    f"-> Detections for {scene} is greater than the maximum: {detection_count[scene]['current']} (max: {detection_count[scene]['maximum']})!"
                )
                return False
