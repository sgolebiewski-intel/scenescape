...
def check_cam_calibration(browser, not_expected_cam=(0, 0), not_expected_map=(0, 0)):
    ...
    try:
        ...
        if (cam_values_init[0] != not_expected_cam) and (
            ...
            print(
                f"Perspective changed to: Cam coord1: '{cam_values_init[0]}' Map coord1: '{map_values_init[0]}'"
            )
            return True
        else:
    ...
    try:
        ...
        for index in range(len(expected_cam_values)):
            if (cam_values_init[index] == expected_cam_values[index]) and (
                ...
                print(
                    f"Calibration persists: Cam coord {index}: '{cam_values_init[index]}', Map coord {index}: '{map_values_init[index]}'"
                )
            else:
                print(f"Calibration for point {index} not as expected:")
                print(
                    f"Cam coord {index} is: '{cam_values_init[index]}', expected: '{expected_cam_values[index]}'"
                )
                print(
                    f"Map coord {index} is: '{map_values_init[index]}', expected: '{expected_map_values[index]}'"
                )
                calibration = False
    except Exception as e:
        print("Error in verifying perspective persistence: ", e)
...
def create_roi(browser, polygon_name, x, y, side_length=250):
    ...
    p_list = list(
        map(float, polygon_points.split(","))
    )  # has the list of vertices of the polygon created
    ...
    if roi_name.get_attribute("value") == polygon_name:
        print(
            f"ROI Name(Text box) updated successfully to {roi_name.get_attribute('value')}"
        )
...
class InteractWith3DScene(InteractWithPage):
    ...
    def unhide_control_panels(self) -> bool:
        ...
        if (
            not camera_3d_controls.is_displayed()
            or not scene_3d_controls.is_displayed()
        ):
            return False
