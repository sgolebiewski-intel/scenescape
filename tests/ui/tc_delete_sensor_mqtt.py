...
def getSensorUid(rest, sensor_name):
    ...
    assert res[
        "results"
    ], f"getSensors REST call hasn't returned any results for {sensor_name}!"
...
def test_sensor_delete_mqtt(params, record_xml_attribute):
    ...
    try:
        ...
        for sensor_type in SENSOR_TYPE_CHOICES:
            ...
            assert (
                message_received
            ), f"The scene hasn't processed any event from the {sensor_type} sensor!"
            exit_code -= 1
            ...
            print(
                "Events should not be received from the sensor because it was deleted..."
            )
            message_received = verify_message_mqtt(client)
            # Test should fail if scene processed any of the sensors tested
            assert (
                not message_received
            ), f"The scene processed events from the {sensor_type} sensor!"
            exit_code -= 1
