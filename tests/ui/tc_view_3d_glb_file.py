...
def upload_3D_scene_asset(browser, file_name, file_path):
    ...
    scene_update_params = common.InteractionParams(
        file_name,
        file_path,
        f"/scene/update/{common.TEST_SCENE_ID}/",
        "",
        "#id_map",
        element_location="",
    )
...
@common.mock_display
@common.scenescape_login_headed
def check_3D_scene_asset_in_3D_scene(
    browser, base_screenshot, file_name, file_path, DEBUG
):
    ...
    """
    scene_3D_params = common.InteractionParams(
        "/media/" + file_name,
        file_path,
        f"/scene/detail/{common.TEST_SCENE_ID}/",
        "",
        "",
        element_location="#map-url",
        element_type="attribute",
        screenshot_threshold=2.75,
        debug=DEBUG,
    )
...
def test_3D_file_upload_visibility(params, record_xml_attribute):
    ...
    try:
        ...
        # glb test
        success_1, success_2 = file_visibility_test(
            params, "box.glb", base_screenshot, DEBUG
        )
        assert success_1
        assert success_2
