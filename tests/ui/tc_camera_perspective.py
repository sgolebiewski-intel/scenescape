...
def test_cam_perspective_main(params, record_xml_attribute):
    ...
    try:
        ...
        assert common.check_cam_calibration(
            browser, cam_values_start[0], map_values_start[0])
        ...
        log.info(
            'Get saved calibration coorinates before temporary change and reset.')
        ...
        changed_perspective = common.change_cam_calibration(
            browser, [10, random_point], [0, 350], False)
        time.sleep(TEST_WAIT_TIME)
        log.info("Get temporary calibration coorinates after change.")
        cam_values_change_temp = common.get_calibration_points(
            browser, 'camera', False)
        map_values_change_temp = common.get_calibration_points(
            browser, 'map', False)
        verified_perspective_change = (
            cam_values_change_temp[0] != cam_values_init[0]) and (
        ...
        cam_values_reset_temp = common.get_calibration_points(
            browser, 'camera', False)
        map_values_reset_temp = common.get_calibration_points(
            browser, 'map', False)

        log.info('Get saved calibration coorinates after reset.')
        cam_values_reset_saved = common.get_calibration_points(
            browser, 'camera')
        map_values_reset_saved = common.get_calibration_points(browser, 'map')

        log.info(
            'Validate if postreset temporary perspective match postreset saved perspective.')
        postreset_saved_perspective = (
            cam_values_reset_temp == cam_values_reset_saved) and (
            map_values_reset_temp == map_values_reset_saved)
        log.info(
            'Validate if postreset saved perspective match inintial perspective.')
        saved_perspective_match_init = (
            cam_values_reset_saved == cam_values_init) and (
