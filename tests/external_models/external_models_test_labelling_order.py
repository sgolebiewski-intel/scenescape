...
def build_argparser():
    ...
    parser.add_argument(
        ...
        "--config",
        type=str,
        help="Config file matching input files vs expected labels.",
        required=True)
    return parser


def compare_expected_to_detections_single_frame(
        expected_detections, frame_detections):
        ...
        if expected['frame'] == frame_detections['frame']:
            ...
            for exp_obj in expected['objects']:
                for det_obj in frame_detections['objects']:
                    ...
                    if np.isclose(
                        det_obj['bounding_box_px']['x'],
                        exp_obj['bounding_box_px']['x'],
                        atol=test_tolerance) and np.isclose(
                        det_obj['bounding_box_px']['y'],
                        exp_obj['bounding_box_px']['y'],
                        atol=test_tolerance) and np.isclose(
                        det_obj['bounding_box_px']['width'],
                        exp_obj['bounding_box_px']['width'],
                        atol=test_tolerance) and np.isclose(
                        det_obj['bounding_box_px']['height'],
                        exp_obj['bounding_box_px']['height'],
                            atol=test_tolerance):
                        if exp_obj['category'] == det_obj['category']:
                            frame_matched_detections += 1

                            print(
                                "Matching Detection vs Expected for frame {} object {}".format(
                                    frame_detections['frame'], det_obj['category']))
                        else:
                            print("Mismatch in category! For frame {} expected {}, got {}".format(
                                frame_detections['frame'], exp_obj['category'], det_obj['category']))
                            frame_matching_failed = True
                            break

            if num_objects != frame_matched_detections:
                print(
                    "Mismatch in number of objects detected (detection positions dont match!)")
                frame_matching_failed = True
                break
            else:
    ...
    with open(test_config['output'], 'r') as output_fd:
        ...
        while not done:
            ...
            if not detection_line:
                ...
                if not compare_expected_to_detections_single_frame(
                        test_config['expected_detections'], detections):
                    print(
                        "Test failed at frame {}".format(
                            detections['frame']))
                    comparison_result = False
...
def test_detection():
    ...
    if os.path.exists(zip_path):
        ...
        if not compare_output_with_detections(test_config):
            ...
            reversed_config_name = 'models/{}/model/reversed_config.json'.format(
                env_model)
            target_config_name = 'models/{}/model/config.json'.format(
                env_model)

        if testResult == 0 and os.path.exists(reversed_config_name):
            # Update the config.json file to point to the reversed one
            update_config_cmd = [
                'mv', reversed_config_name, target_config_name]
