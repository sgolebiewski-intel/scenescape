...
parser.add_argument(
    "--compare_mse",
    action="store_true",
    help="Compare Mean Square Error")
parser.add_argument(
    "--mse_error_margin",
    type=float,
    default=0.0,
    help="Mean Square Error Margin")
parser.add_argument(
...
parser.add_argument(
    "--compare_only_average",
    action="store_true",
    help="Compare only the average error")
...
def compareMse(precalculated_errors, recalculated_errors, mse_error_margin):
    for frame in precalculated_errors:
        ...
        if recalculated_errors[frame]["mean_square_error"] is None:
            print(
                f"Frame: {frame} : Mean Square Error not calculated",
                flush=True)
            continue
        print(
            f"Mean Square Error for Frame: {frame} : Precalculated {
                precalculated_errors[frame]['mean_square_error']} / Recalculated {
                recalculated_errors[frame]['mean_square_error']}",
            flush=True)
        if abs(
                precalculated_errors[frame]["mean_square_error"] -
                recalculated_errors[frame]["mean_square_error"]) > mse_error_margin:
            return False
    return True


def compareReprojectionError(
        precalculated_errors,
        recalculated_errors,
        reprojection_error_margin):
    for frame in precalculated_errors:
        if precalculated_errors[frame]["image_recieved"] == "false":
        ...
        if recalculated_errors[frame]["reprojection_error"] is None:
            print(
                f"Frame: {frame} : Reprojection Error not calculated",
                flush=True)
            continue
        print(
            f"Reprojection Error for Frame: {frame} : Precalculated {
                precalculated_errors[frame]['reprojection_error']} / Recalculated {
                recalculated_errors[frame]['reprojection_error']}",
            flush=True)
        if abs(
                precalculated_errors[frame]["reprojection_error"] -
                recalculated_errors[frame]["reprojection_error"]) > reprojection_error_margin:
            return False
    return True
...
def compareAverageMseError(
    ...
    if abs(
            ...
            recalculated_average_error["mean_square_error"]) > mse_error_margin:
        return False
    return True
...
def compareAverageReprojectionError(
    ...
    if abs(
            ...
            recalculated_average_error["reprojection_error"]) > reprojection_error_margin:
        return False
    return True
...
def calculateAverageErrors(errors_dict):
    ...
    average_error = {
        "mean_square_error": total_mse /
        total_mse_frames if total_mse_frames > 0 else None,
        "reprojection_error": total_reprojection_error /
        total_reprojection_error_frames if total_reprojection_error_frames > 0 else None}
    ...
    if args.compare_only_average:
        if average_mse_comparison and average_reprojection_error_comparison:
            ...
            if args.compare_mse and not average_mse_comparison:
                print(
                    "Average Mean Square Errors are not within the margin",
                    flush=True)
                TEST_PASS = False
            if args.compare_reprojection_error and not average_reprojection_error_comparison:
                print(
                    "Average Reprojection Errors are not within the margin",
                    flush=True)
                TEST_PASS = False
    else:
        if mse_error_comparison and reprojection_error_comparison:
            ...
            if args.compare_reprojection_error and not reprojection_error_comparison:
                print(
                    "Reprojection Errors are not within the margin",
                    flush=True)
                TEST_PASS = False
    print(
        f"Average Mean Square Error: {
            recalculated_average_error['mean_square_error']}",
        flush=True)
    print(
        f"Average Reprojection Error: {
