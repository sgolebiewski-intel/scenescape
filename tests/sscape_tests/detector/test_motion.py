...
@pytest.mark.parametrize(
    "motion_obj, camera_id, expected_bboxes",
    [
        (
            "motion_detector",
            "camera1",
            [{"x": 837, "y": 0, "width": 146, "height": 169}],
        ),
        (
            detector_motion.MotionMog2Detector(),
            "camera1",
            [{"x": 837, "y": 0, "width": 146, "height": 169}],
        ),
        (
            detector_motion.MotionKnnDetector(),
            "camera2",
            [{"x": 834, "y": 0, "width": 151, "height": 253}],
        ),
        (None, "camera3", [{"x": 837, "y": 0, "width": 146, "height": 169}]),
    ],
)
def test_detect(
    motion_detector, motion_frames, motion_obj, camera_id, expected_bboxes, request
):
...
def test_preprocess(motion_detector, motion_frames):
    ...
    assert grayscale[0].shape == (
        motion_frames[0].data[0].shape[0],
        motion_frames[0].data[0].shape[1],
    )
