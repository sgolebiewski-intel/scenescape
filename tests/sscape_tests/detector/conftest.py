...
def reid_postprocessed_data(reid_detector, input_data, reid_preprocessed_data):
    ...
    reid_detector.startInfer(
        reid_preprocessed_data[0],
        input_data.id,
        debugFlag=False)
...
def mock_result(mock_class):
    ...
    mock_class.corners = np.asarray(
        [[87, 230], [39, 237], [23, 203], [73, 197]])
    mock_class.tag_id = 1
...
def ovms_hpe():
    ...
    detector_obj = detector.PoseEstimator(
        distributed=detector.Distributed.OVMS)
    detector_obj.setParameters(
        ovms_hpe_model,
        device,
        plugin,
...
def ovms_geti():
    ...
    ovms_geti_obj = detector_geti.GetiDetector(
        distributed=detector_geti.Distributed.OVMS)
    ovms_geti_obj.setParameters(
        ovms_retail_model,
        device,
        plugin,
        threshold,
