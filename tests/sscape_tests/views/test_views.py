...
from manager.views import (
    SingletonSensorDeleteView,
    SingletonSensorCreateView,
    SingletonSensorUpdateView,
    CamCreateView,
    CamDeleteView,
    CamUpdateView,
)
...
class SetUpTestCases(TestCase):
    def setUp(self):
        ...
        SingletonSensor.objects.create(
            sensor_id="100", name="test_sensor", scene=test_scene
        )
...
class TestRoiViews(TestCase):
    ...
    def test_save_ROIs(self):
        response = self.client.post(
            reverse("save-roi", args=[self.test_scene_id]),
            data={
                "rois": json.dumps(
                    [
                        {
                            "title": "roi1",
                            "points": [[1, 2]],
                            "uuid": "5d03455d-82e6-4d3c-abc2-a496c43e4d53",
                        }
                    ]
                ),
                    ...
                    [
                        {
                            "title": "trip1",
                            "points": [[1, 2]],
                            "uuid": "9029524e-b764-438e-912e-9613d43895a0",
                        }
                    ]
                ),
            },
...
class TestCameraViews(TestCase):
    ...
    def test_camera_calibrate_elif_1(self):
        dummy = self.camera_intrinsics.copy()
        dummy.update(
            {"save_camera_details": 1, "scene": "1", "name": "camera1", "sensor_id": 1}
        )
        response = self.client.post("/cam/calibrate/1", data=dummy)
        self.assertEqual(response.status_code, 200)
        return
