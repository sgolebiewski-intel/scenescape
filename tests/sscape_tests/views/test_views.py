...
class TestRoiViews(TestCase):
    ...
    def test_save_ROIs(self):
        response = self.client.post(reverse('save-roi',
                                            args=[self.test_scene_id]),
                                    data={'rois': json.dumps([{'title': 'roi1',
                                                               'points': [[1,
                                                                           2]],
                                                               'uuid': '5d03455d-82e6-4d3c-abc2-a496c43e4d53'}]),
                                          'tripwires': json.dumps([{'title': 'trip1',
                                                                    'points': [[1,
                                                                                2]],
...
class TestSingletonSensorViews(TestCase):
    ...
    def test_generic_calibrate(self):
        ...
        response = self.client.post(
            '/singleton_sensor/calibrate/1', data=dummy)
        self.assertEqual(response.status_code, 200)
        return
