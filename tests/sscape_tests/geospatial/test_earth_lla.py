...
def lla_datafile(tmp_path):
    ...
    for i in range(len(equator_points_data["pixels per meter"])):
        ...
        angle_lat_deg = 0.5 * np.rad2deg(
            equator_points_data["map_dim_x"][i] / earth_lla.EQUATORIAL_RADIUS
        )
        angle_long_deg = 0.5 * np.rad2deg(
            equator_points_data["map_dim_y"][i] / earth_lla.EQUATORIAL_RADIUS
        )
        ...
        map_resolution = [
            int(
                equator_points_data["map_dim_x"][i]
                * equator_points_data["pixels per meter"][i]
                * equator_points_data["scale"][i]
            ),
            int(
                equator_points_data["map_dim_y"][i]
                * equator_points_data["pixels per meter"][i]
                * equator_points_data["scale"][i]
            ),
        ]
        inputs.append(
            {
                ...
                "lat, long, altitude points": [
                    ...
                    [
                        -angle_lat_deg,
                        map_center_long + angle_long_deg,
                        altitude + z_shift,
                    ],
                    [
                        angle_lat_deg,
                ...
                "map points": [
                    ...
                    [
                        map_resolution[0] / pixels_per_meter,
                        map_resolution[1] / pixels_per_meter,
                        0,
                    ],
                    ...
                    # center point elevated
                    [
                        0.5 * map_resolution[0] / pixels_per_meter,
                        0.5 * map_resolution[1] / pixels_per_meter,
                        z_shift,
                    ],
                ],
            }
...
def test_convertLLAToECEF():
    ...
    test_inputs = [[0, 0, 100], [0, 90, 0], [90, 0, 0]]
    expected_outputs = np.array([[a + 100, 0, 0], [0, a, 0], [0, 0, b]])
...
def test_convertECEFToLLA():
    a = earth_lla.EQUATORIAL_RADIUS
    b = earth_lla.POLAR_RADIUS
    test_inputs = np.array([[a + 100, 0, 0], [0, a, 0], [0.1, 0, b]])
    expected_outputs = np.array([[0, 0, 100], [0, 90, 0], [90, 0, 0]])
    for i, ti in enumerate(test_inputs):
...
def test_convertToCartesianTRS():
    ...
    rotMat = sstr.Rotation.from_euler(
        "XYZ", list(180 * np.random.rand(3)), degrees=True
    ).as_matrix()
...
def test_getConversionBothWays():
    ...
    """
    rand_vec = np.random.rand(int(1e4), 3)
...
def calcLLAError(lla_pt_1, lla_pt_2):
    ...
    return np.linalg.norm(
        earth_lla.convertLLAToECEF(lla_pt_1) - earth_lla.convertLLAToECEF(lla_pt_2)
    )
...
def test_calculateTRSLocal2LLAFromSurfacePoints(lla_datafile):
    ...
    for input in inputs:
        ...
        trs_mat = earth_lla.calculateTRSLocal2LLAFromSurfacePoints(
            map_pts[:4], lla_pts[:4]
        )
        ...
        trs_mat = earth_lla.calculateTRSLocal2LLAFromSurfacePoints(
            map_pts[:4], lla_pts[:4], z_shift=1.0
        )
        for i, pt in enumerate(map_pts):
            calc_lla_pt = earth_lla.convertXYZToLLA(trs_mat, pt)
            error = calcLLAError(calc_lla_pt, lla_pts[i])
    ...
    with pytest.raises(ValueError) as excinfo:
        ...
        earth_lla.calculateTRSLocal2LLAFromSurfacePoints(
            map_pts[:4] + np.ones([4, 3]), lla_pts[:4]
        )
...
def test_calculateTRSLocal2LLAFromImageMap(lla_datafile):
    ...
    for input in inputs:
        ...
        # test with 4 points and default z_shift
        trs_mat = earth_lla.calculateTRSLocal2LLAFromImageMap(
            resx, resy, input["pixels per meter"], lla_pts[:4]
        )
        ...
        # test with 4 points and z_shift 1.0 meter
        trs_mat = earth_lla.calculateTRSLocal2LLAFromImageMap(
            resx, resy, input["pixels per meter"], lla_pts[:4], z_shift=1.0
        )
        for i, pt in enumerate(input["map points"]):
            calc_lla_pt = earth_lla.convertXYZToLLA(trs_mat, pt)
            error = calcLLAError(calc_lla_pt, lla_pts[i])
    ...
    with pytest.raises(ValueError) as excinfo:
        # 3 lla points
        earth_lla.calculateTRSLocal2LLAFromImageMap(
            resx, resy, input["pixels per meter"], lla_pts[:3]
        )
    assert (
        "number of map points must match number of geographic points"
        in str(excinfo.value).lower()
...
def test_getHeading():
    ...
    test_inputs = np.array(
        [[[a, 0, 0], [0, 0, 1]], [[a, 0, 0], [0, 1, 0]], [[a, 0, 0], [0, 1, 1]]]
    )
    expected_outputs = np.array([0, 90, 44.808])
    for i, ti in enumerate(test_inputs):
