...
def lla_datafile(tmp_path):
    ...
    for i in range(len(equator_points_data['pixels per meter'])):
        ...
        angle_lat_deg = 0.5 * \
            np.rad2deg(equator_points_data['map_dim_x'][i] / earth_lla.EQUATORIAL_RADIUS)
        angle_long_deg = 0.5 * \
            np.rad2deg(equator_points_data['map_dim_y'][i] / earth_lla.EQUATORIAL_RADIUS)
        ...
        map_resolution = [int(equator_points_data['map_dim_x'][i] *
                              equator_points_data['pixels per meter'][i] *
                              equator_points_data['scale'][i]), int(equator_points_data['map_dim_y'][i] *
                                                                    equator_points_data['pixels per meter'][i] *
        ...
        inputs.append({
            ...
            "lat, long, altitude points": [
                ...
                [angle_lat_deg, map_center_long - angle_long_deg, altitude],
                [- angle_lat_deg, map_center_long - angle_long_deg, altitude],
                [- angle_lat_deg, map_center_long +
                    angle_long_deg, altitude + z_shift],
                [angle_lat_deg, map_center_long +
                    angle_long_deg, altitude + z_shift],
                [angle_lat_deg, map_center_long -
                    angle_long_deg, altitude + z_shift],
            ...
            "map points": [
                ...
                [map_resolution[0] / pixels_per_meter,
                    map_resolution[1] / pixels_per_meter, 0],
                [0, map_resolution[1] / pixels_per_meter, 0],
                [0, 0, z_shift],
                [map_resolution[0] / pixels_per_meter, 0, z_shift],
                [map_resolution[0] /
                 pixels_per_meter, map_resolution[1] /
                 pixels_per_meter, z_shift],
                [0, map_resolution[1] / pixels_per_meter, z_shift],
                # center point elevated
                [0.5 * map_resolution[0] / pixels_per_meter, 0.5 *
                    map_resolution[1] / pixels_per_meter, z_shift],
            ]
        })
...
def test_convertToCartesianTRS():
    ...
    pts2 = scale * pts1
    pts2 = np.array([np.matmul(rotMat, pts2[i, :])
                    for i in range(pts2.shape[0])])
    pts2 = pts2 + translation
    trs_mat = earth_lla.convertToCartesianTRS(pts1, pts2)

    calc_pt = np.matmul(np.linalg.inv(trs_mat),
                        np.hstack([pts2[0, :], 1]).T)[0:3]
...
def test_getConversionBothWays():
    ...
    for i, pt in enumerate(rand_vec):
        ...
        pt_lla = pt * np.array([180.0, 180.0, 100.0]) - \
            np.array([90.0, 90.0, 50.0])
...
def calcLLAError(lla_pt_1, lla_pt_2):
    ...
    return np.linalg.norm(
        earth_lla.convertLLAToECEF(lla_pt_1) -
        earth_lla.convertLLAToECEF(lla_pt_2))
...
def test_calculateTRSLocal2LLAFromSurfacePoints(lla_datafile):
    ...
    with pytest.raises(ValueError) as excinfo:
        ...
        earth_lla.calculateTRSLocal2LLAFromSurfacePoints(
            map_pts[:3], lla_pts[:4])
    assert "number of map points must match number of geographic points" in str(
        excinfo.value).lower()
