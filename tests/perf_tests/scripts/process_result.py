...
def compare_item(jdata, jref, precision):
    ...
    if not isinstance(jdata, type(jref)):
        print(
            "{} is of a different type between data ({}) and ref ({})".format(
                jdata,
                type(jdata),
                type(jref)))
        return False
    ...
    if isinstance(jdata, float):
        ...
        if not comparison_result:
            print("{} v {} : {} tol {} precision {}".format(
                jdata, jref, comparison_result, tol, precision))
    ...
    if isinstance(jdata, list):
        if len(jdata) != len(jref):
            print(
                "{} is of a different length in data ({}) and ref ({})".format(
                    jdata, len(jdata), len(jref)))
            return False
    ...
    with open(parser.input, "r") as file_data:
        with open(parser.reference, "r") as file_ref:
            ...
            while not done:
                ...
                if line_data is None or line_ref is None or len(
                        line_data) == 0 or len(line_ref) == 0:
                    ...
                    result = process_line(
                        line_data, line_ref, parser.precision)

                    if not result:
                        print(
                            "Comparing line {} result {}".format(
                                current_line, result))
                        done = True
