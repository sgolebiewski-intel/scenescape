...
class Cybertronium:
    ...
    def findCoord(self, x, y, scale, dotScale, points):
        ...
        for idx in range(len(points)):
            ...
            if abs(c[0] - xs) <= 5 * \
                    dotScale and abs(c[1] - ys) <= 5 * dotScale:
                return idx
        return None
    ...
    def handleEvent(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            self.moving = self.findCoord(
                x, y, self.scale, self.dotScale, self.coords)
            if self.moving is not None:
                self.offset = (self.coords[self.moving][0] - x * self.scale,
                               self.coords[self.moving][1] - y * self.scale)
            elif self.rcoords is not None:
                rpoint = self.findCoord(
                    x, y, self.scale, self.dotScale, self.rcoords)
                print("REGION POINT", rpoint)
        elif event == cv2.EVENT_LBUTTONUP:
            self.moving = None
            param.map.display(
                cameraOrigin=param.cameraOrigin,
                regionOfView=param.regionOfView)
...
class Autobot:
    def __init__(
        ...
        self.frame = Cybertronium(
            self.number,
            self.warpName,
            image,
            fcoords,
            self.intrinsics)
        self.map = Cybertronium(
            self.number,
            self.mapName,
            mapImage,
            mcoords,
    ...
    def scaleFieldOfView(self, coords, scale):
        self.regionOfView = [
            tuple(
                c *
                scale for c in x.as2Dxy.cv) for x in coords]
    ...
    def overlayMap(self):
        ...
        if self.doOverlay:
            ...
            if self.rcoords is not None:
                for idx, coord in enumerate(self.rcoords):
                    idxn = (idx + 1) % len(self.rcoords)
                    coordn = self.rcoords[idxn]
                    wcoord = self.transformPoint(
                        coord.asNumpyCartesian, transformMap)
                    wcoordn = self.transformPoint(
                        coordn.asNumpyCartesian, transformMap)
                    scl_line(
                        overlay, wcoord.cv, wcoordn.cv, (0, 0, 128 + 64), 3)
        else:
            overlay = self.createOverlay(
                self.frame.image, self.map.image, transformFrame)
        ...
        scl_line(overlay, (overlay.shape[1] - 1, 0),
                 self.cameraOrigin.cv, (0, 255, 0), 4)
                ...
                for coord in self.rcoords:
                    wcoords.append(
                        self.transformPoint(
                            coord.point,
                            transformMap).point)
                self.rmoving = self.frame.findCoord(
                    x, y, self.frame.scale, self.frame.dotScale, wcoords)

                if self.rmoving is not None:
                    self.offset = (wcoords[self.rmoving][0] -
                                   x *
                                   self.frame.scale, wcoords[self.rmoving][1] -
                                   y *
                                   self.frame.scale)
                    return
...
def main():
    ...
    print('"intrinsics": %s,' %
          (json.dumps(pos1.intrinsics.intrinsics.tolist())))
