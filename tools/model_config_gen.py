...
def warn_entry(obj, entry):
    print(
        "  Warning entry", entry, "was generated with dummy values (", obj[entry], ")."
    )
...
def resolve_container_path(args_directory, args_name):
    ...
    g_models_path = os.path.realpath(
        os.path.join(os.path.dirname(config_script_path), "../models")
    )
...
def main():
    ...
    print(
        "1) In the volume subsection, ensure the following volume pair description is present"
    )
    ...
    for dirpath, dirnames, filenames in os.walk(args.directory):
        ...
        for filename in filenames:
            ...
            if found_model_xml and found_model_bin:
                ...
                mconfig = create_config(
                    dirpath, found_geti, model_config_name, found_xml_file
                )
                ...
                if not g_requested_path.is_relative_to(g_models_path):
                    volume_extra_str = (
                        f"--volume ./{g_volume_path}:{g_container_base_path}"
                    )
                print(f"    $ tools/scenescape-start {volume_extra_str} --shell")
                print(
                    f"    $ percebro/percebro -i sample_data/apriltag-cam1.mp4 --modelconfig {args.output} --intrinsics=70 --debug --stats --frames 200 -m {model_config_name}"
                )
