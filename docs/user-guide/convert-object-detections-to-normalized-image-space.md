...
Once all the pieces are in place, Intel® Edge Spatial Intelligence can essentially "project" detected objects onto the scene and apply its tracking system to update the associated nodes in the scene graph.

Intel® Edge Spatial Intelligence's tracking system does not directly ingest images or video. Instead, it consumes the output of a video analytics pipeline. This has several key advantages:

1. **Privacy**: Only the bounding box and other detection metadata are sent to Intel® Edge Spatial Intelligence, not the actual images (except for camera calibration and preview purposes)
...
3. **Various Detection Technologies**: Many models and methods exist for object/person detection using images, video, and other image-like data (e.g. thermal, radar, infrared), and Intel® Edge Spatial Intelligence should be able to utilize these systems and modalities
...
**Note**: Only the green dot representing the detection location is shown since Intel® Edge Spatial Intelligence does not have access to the source image.
...
Intel® Edge Spatial Intelligence utilizes these detections from various viewpoints to update the associated node in the scene graph. Note that the detections from various sources arrive asynchronously, so source timestamps and time coordination are critical to effective tracking.
...
> **Note:** Intel® Edge Spatial Intelligence also provides some helper classes and methods for transforming data from pixel space to normalized image space. Intel® Edge Spatial Intelligence includes [scene_common/transform.py](https://github.com/open-edge-platform/scenescape/blob/main/scene_common/src/transform.py) to provide various methods for handling these transformations.
