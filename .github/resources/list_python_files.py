...
def main():
    ...
    cmd_find_files_with_shebang = [
        "grep", "-rwl", ".", "-e", "#!/usr/bin/env python3"]

    files_with_py_extension = subprocess.run(
        cmd_find_py_files, stdout=subprocess.PIPE)
    ...
    all_python_files = files_with_py_extension.stdout.decode(
        'utf-8') + python_files_without_extension.stdout.decode('utf-8')
