...
function computeVerticalFOVFromDiagonal(dFOV, resolution) {
  const hypotenuse = Math.sqrt(resolution.w * resolution.w + resolution.h * resolution.h);
  const temp = Math.tan(THREE.MathUtils.degToRad(dFOV / 2)) * (resolution.h / hypotenuse);
  return 2 * THREE.MathUtils.radToDeg(Math.atan(temp));
}

function computeDiagonalFOV(cx, cy, fy) {
  return 2 * THREE.MathUtils.radToDeg(Math.atan(Math.sqrt(cx * cx + cy * cy) / fy));
}
...
function constructIntrinsicsMatrix(intrinsics, resolution) {
  if (['fx', 'fy', 'cx', 'cy'].every(key => key in intrinsics)) {
    return [[intrinsics['fx'], 0, intrinsics['cx']],
    [0, intrinsics['fy'], intrinsics['cy']],
    [0, 0, 1]];
  }
  else if ('fov' in intrinsics) {
    return [[computeFyFromDFOV(intrinsics['fov'], resolution), 0, resolution.w / 2],
    [0, computeFyFromDFOV(intrinsics['fov'], resolution), resolution.h / 2],
    [0, 0, 1]];
  }

  return [[intrinsics[0], 0, intrinsics[2]],
...
export default class SceneCamera extends THREE.Object3D {
  constructor(params) {
    ...
    this.intrinsics = 'intrinsics' in params ? params.intrinsics : DEFAULT_INTRINSICS;
    this.distortion = 'distortion' in params ? params.distortion : DEFAULT_DISTORTION;
    this.resolution = 'resolution' in params ? { w: params.resolution[0], h: params.resolution[1] } : DEFAULT_RESOLUTION;
    this.cameraMatrix = cv.matFromArray(3, 3, cv.CV_64F, constructIntrinsicsMatrix(this.intrinsics, this.resolution).flat());
    this.distCoeffs = cv.matFromArray(1, 5, cv.CV_64F, constructDistortionArray(this.distortion));
    this.imagePoints = null;
    this.calibPoints = new THREE.Group();
    this.cameraPosition = 'translation' in params ? new THREE.Vector3(...params.translation) : new THREE.Vector3(0, 0, 0);
    this.cameraRotation = 'rotation' in params ? new THREE.Euler(...params.rotation.map(function (item) {
    ...
    if (this.cameraPosition && this.cameraRotation) {
      ...
      this.toast.showToast(`Failed to load position/rotation for ${this.name}. Check intrinsics config in percebro.`, 'warning');
    }
  }

  addCamera() {
    const vfov = computeVerticalFOVFromFy(this.cameraMatrix.data64F[FY], this.resolution);
    this.sceneCamera = new THREE.PerspectiveCamera(vfov, this.resolution.w / this.resolution.h, 0.1, 10);
  ...
  addObject(params) {
    ...
    this.addDragControls(params.sceneViewCamera, params.orbitControls, () => {
      ...
      this.imagePoints = null;
      this.executeOnControl('fov', (control) => { control[0].domElement.classList.add('disabled') });
      this.fovEnabled = false;
    });

    const fields = ['name', 'scene camera', 'show camera', 'project frame',
      'pause video', 'opacity', 'toggle rotate/translate',
  ...
  updateDistortion(distortion) {
    if (distortion) {
      this.distCoeffs = cv.matFromArray(1, 5, cv.CV_64F, constructDistortionArray(distortion));
      this.distortionFolder.children[0].$input.value = this.distCoeffs.data64F[K1];
    }
    return
  }
  updateIntrinsics(intrinsics) {
    ...
    if (JSON.stringify(newIntrinsics) !== JSON.stringify(this.intrinsics)) {
      ...
      this.cameraMatrix = cv.matFromArray(3, 3, cv.CV_64F, constructIntrinsicsMatrix(this.intrinsics, this.resolution).flat());
      this.reloadCamera();
      this.intrinsicsFolder.children[0].$input.value = this.cameraMatrix.data64F[FX];
      this.intrinsicsFolder.children[1].$input.value = this.cameraMatrix.data64F[FY];
      this.intrinsicsFolder.children[2].$input.value = this.cameraMatrix.data64F[CX];
      this.intrinsicsFolder.children[3].$input.value = this.cameraMatrix.data64F[CY];
    }

    this.executeOnControl('fov', (control) => {
      let dfov = computeDiagonalFOV(this.cameraMatrix.data64F[CX],
        this.cameraMatrix.data64F[CY],
        this.cameraMatrix.data64F[FY]);
      if (dfov !== control[0].getValue()) {
        control[0].setValue(
          computeDiagonalFOV(this.cameraMatrix.data64F[CX],
            this.cameraMatrix.data64F[CY],
            this.cameraMatrix.data64F[FY]));
      }
    });
  ...
  addControlPanel(camerasFolder) {
    ...
    let panelSettings = {
      ...
      'opacity': 100,
      'fov': computeDiagonalFOV(this.cameraMatrix.data64F[CX], this.cameraMatrix.data64F[CY], this.cameraMatrix.data64F[FY]),
      'scene camera': false,
      'calibration points visibility': this.calibPoints && this.calibPoints.visible,
      ...
      'auto calibrate': (function () { this.autoCalibrate() }).bind(this),
      'show camera': true,
      'color': "#ffffff",
      'aspect ratio': this.aspectRatio,
      'save': (function () { this.saveSettings() }).bind(this),
      'delete': (function () { this.removeFromScene(camerasFolder) }).bind(this),
    };

    let control = this.controlsFolder.add(panelSettings, 'name').onChange((function (value) {
      this.prevName = this.name;
      this.name = value;
      this.validateField('name', () => { return this.name === '' || this.name === DEFAULT_CAMERA_NAME; });
    }).bind(this));
    control.$widget.firstChild.id = this.name.concat('-', 'name');

    control = this.controlsFolder.add(panelSettings, 'scene camera').onChange((function (setAsDefault) {
    ...
    control = this.controlsFolder.add(panelSettings, 'show camera').onChange((function (value) {
      this.visible = value;
    }).bind(this));

    control = this.controlsFolder.add(panelSettings, 'calibration points visibility').onChange((function (visibility) {
      this.calibPoints.visible = visibility;
    }).bind(this));
    control.$widget.firstChild.id = this.name.concat('-', 'calibration');
    if (!(this.calibPoints && this.calibPoints.children.length > 0)) control.hide();

    control = this.controlsFolder.add(panelSettings, 'project frame').onChange((function (visibility) {
      this.projectFrame = visibility;
      if (this.projectFrame) {
        this.mqttClient.publish(this.appName + CMD_CAMERA + this.name, 'getimage');
      }
    ...
    control = this.controlsFolder.add(panelSettings, 'pause video').onChange((function (visibility) {
    ...
    control = this.controlsFolder.add(panelSettings, 'opacity', 0, 100, 1).onChange((function (weight) {
      if (this.cameraCapture) this.cameraCapture.opacity = weight / 100.0;
    }).bind(this));
    control.$input.id = this.name.concat('-', 'opacity');

    control = this.controlsFolder.add(panelSettings, 'fov', 1, 180, 1).onChange((function (fov) {
      this.fovEnabled = true;
      let newIntrinsics = constructIntrinsicsMatrix({ 'fov': fov }, this.resolution);
    ...
    control = this.controlsFolder.addColor(panelSettings, 'color').onChange((function (value) {
      this.projectionColor = value.toUpperCase();
      this.generateProjectionFrame();
    }).bind(this));

    control = this.controlsFolder.add(panelSettings, 'aspect ratio').onFinishChange((function (value) {
    ...
    if (this.name === '' || this.name === DEFAULT_CAMERA_NAME) this.addFieldWarning('name');
  ...
  generateProjectionFrame() {
    ...
    const image = this.createSolidColorPng(this.projectionColor, this.aspectRatio);
  ...
  enableAutoCalibration(value) {
    if (value) {
      this.executeOnControl('auto calibrate', (control) => { control[0].domElement.classList.remove('disabled') });
    }
    else {
      this.executeOnControl('auto calibrate', (control) => { control[0].domElement.classList.add('disabled') });
    }
  }

  hideAutoCalibrateButton() {
    this.executeOnControl('auto calibrate', (control) => { control[0].domElement.hidden = true });
  }
  ...
  addDistortionControls(control, panelSettings) {
    ...
    control = this.distortionFolder.add(panelSettings, 'k1').onFinishChange((function (value) {
  ...
  setCameraVerticalFOV() {
    if (this.resolution) {
      this.sceneCamera.fov = computeVerticalFOVFromFy(this.cameraMatrix.data64F[FY], this.resolution);
  ...
  setCameraAspectRatio() {
    this.sceneCamera.aspect = this.cameraMatrix.data64F[CX] / this.cameraMatrix.data64F[CY];
    this.sceneCamera.updateProjectionMatrix();
    if (this.cameraCapture) {
      this.cameraCapture.camera = this.sceneCamera;
      this.cameraCapture.project(this.mesh);
  ...
  async removeFromScene(camerasFolder) {
    const isConfirmed = window.confirm(`Are you sure you want to remove "${this.name}"?`);
    ...
    if (isConfirmed) {
      ...
      if (this.isStoredInDB) {
        ...
        if (deleteResponse.statusCode == '200') {
          ...
          this.toast.showToast('Camera has been deleted from the database!', 'success');
        }
        else {
          this.scene.add(this);
          this.toast.showToast('Failed to delete the camera from the database!', 'danger');
        }
      } else {
        this.controlsFolder.destroy();
    ...
    this.mqttClient.on('message', (topic, message) => {
      if (topic === this.appName + IMAGE_CAMERA + this.name) {
        ...
        this.isUpdatedInPercebro = compareIntrinsics(
          this.intrinsics, msg.intrinsics.flat(), this.distortion, msg.distortion);
      }
    });
  }
  ...
  autoCalibrate() {
    ...
    this.toast.showToast('Performing auto camera calibration for ' + this.name + '...', 'information', this.name + '-Calibrate', MAX_CALIB_WAIT_TIME);
  ...
  async saveSettings() {
    ...
    if (this.cameraUID) {
      ...
      let waitTime = 0;
      while (!this.isUpdatedInPercebro && waitTime < MAX_INTRINSICS_UPDATE_WAIT_TIME) {
        await new Promise(resolve => setTimeout(resolve, 100));
        waitTime += 100;
      }
      if (!this.isUpdatedInPercebro) {
        const message = `New camera intrinsics did not update in percebro within ` +
                        `${MAX_INTRINSICS_UPDATE_WAIT_TIME}ms.`
        this.toast.showToast(message, 'danger');
      }
    }

    let textObject = this.sceneCamera.getObjectByName("textObject_" + this.previousName);
  ...
  updateCameras(oldCamName, newCamName, textObject) {
    ...
    this.controlsFolder.$title.setAttribute('id', newCamName + '-control-panel');
    this.addStatusIndicator()
  }
  ...
  async createNewCamera(cameraData, textObject) {
    ...
    if (createResponse.statusCode === 201) {
      ...
      this.toast.showToast('Something went wrong. Please make sure the camera name you entered is unique.', 'danger');
    }
    else {
      console.log(createResponse.errors);
      this.toast.showToast('Something went wrong. Failed to save camera setting. Please try again!', 'danger');
    }
  }

  async updateExistingCamera(cameraData) {
    let updateResponse = await this.restclient.updateCamera(this.cameraUID, cameraData);
    if (updateResponse.statusCode === 200) {
      ...
      console.log(updateResponse.errors);
      this.isUpdatedInDB = false;
      this.toast.showToast('Something went wrong. Failed to update camera setting. Please try again!', 'danger');
    }
  }
  ...
  projectCameraCapture(image, data, online = true) {
    if (online) {
      ...
      this.executeOnControl('aspect ratio', (control) => { if (control[0]) control[0].destroy(); });
      this.executeOnControl('color', (control) => { if (control[0]) control[0].destroy(); });
    ...
    if (this.sceneCamera && this.projectFrame && !this.pauseVideo) {
      ...
      loader.load(image, (texture) => {
        ...
        if (this.cameraCapture === null) {
          ...
          ] = this.drawObj.createProjectionMaterial(this.sceneCamera, this.sceneMesh, texture);
  ...
  showCameraOnline() {
    const statusIndicator = this.controlsFolder.$title.querySelector('.offline');
    if (statusIndicator) {
      statusIndicator.classList.replace('offline', 'online');
      this.executeOnControl('project frame', (control) => control[0].$input.removeAttribute("disabled"));
    }
  }
  ...
  updateImagePoints() {
    ...
    this.imagePoints = cv.matFromArray(MAX_CALIB_POINTS, 2, cv.CV_64F, imgPoints);
  }
  ...
  computeTransformMatrix(objPoints) {
    const objectPoints = cv.matFromArray(MAX_CALIB_POINTS, 3, cv.CV_64F, objPoints);
  ...
  onDoubleClick(sphere) {
    ...
    if (this.calibPoints.children.length === MAX_CALIB_POINTS) {
      if (this.imagePoints === null) {
        this.executeOnControl('fov', (control) => { control[0].domElement.classList.remove('disabled') });
