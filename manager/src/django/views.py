...
def cameraCalibrate(request, sensor_id):
    ...
    return render(
        request, "cam/cam_calibrate.html", {"form": form, "caminst": cam_inst}
    )
    ...
    def get_queryset(self):
        ...
        if query_params:
            ...
            bad_keys = [
                x
                for x in keys
                if x not in ("name", "parent", "scene", "username", "id")
            ]
            if bad_keys:
    ...
    for c in children:
        if c.child_type == "local":
            ...
            current_child_sensors = [
                json.loads(s.areaJSON()) | {"title": s.name}
                for s in child_scene_sensors
            ]
