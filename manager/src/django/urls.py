...
urlpatterns = [
    ...
    path(
        "singleton_sensor/list/",
        SingletonSensorListView.as_view(),
        name="singleton_sensor_list",
    ),
    path(
        "singleton_sensor/create/",
        SingletonSensorCreateView.as_view(),
        name="singleton_sensor_create",
    ),
    path(
        "singleton_sensor/detail/<int:pk>/",
        SingletonSensorDetailView.as_view(),
        name="singleton_sensor_detail",
    ),
    path(
        "singleton_sensor/update/<int:pk>/",
        SingletonSensorUpdateView.as_view(),
        name="singleton_sensor_update",
    ),
    path(
        "singleton_sensor/delete/<int:pk>/",
        SingletonSensorDeleteView.as_view(),
        name="singleton_sensor_delete",
    ),
    path(
        "singleton_sensor/calibrate/<int:sensor_id>",
        views.genericCalibrate,
        name="singleton_sensor_calibrate",
    ),
    ...
    re_path(
        r"^%s(?P<path>.*)$" % settings.DOCS_URL[1:],
        views.protected_media,
...
if settings.KUBERNETES_SERVICE_HOST:
    urlpatterns += [
        ...
        re_path(
            r"^%s(?P<path>.*)$" % settings.MODEL_URL[1:],
            views.protected_media,
...
urlpatterns += [
    ...
    re_path(
        r"api/v1/(scene)/([0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12})$",
        views.ManageThing.as_view(),
    ),
...
if settings.KUBERNETES_SERVICE_HOST:
    urlpatterns += [
        path(
            "api/v1/model-directory/", ModelDirectory.as_view(), name="model-directory"
        ),
    ]
