...
class KubeClient:
    ...
    def save(self, msg):
        ...
        if msg.get("distortion_k1", "") not in ["", None]:
            advanced_args.append(
                f"--distortion=[{msg['distortion_k1']},{msg['distortion_k2']},{msg['distortion_p1']},{msg['distortion_p2']},{msg['distortion_k3']}]"
            )
    ...
    def generateDeploymentBody(self, msg, args):
        ...
        volumes = [
            client.V1Volume(
                name="certs",
                secret=client.V1SecretVolumeSource(secret_name=f"{self.release}-certs"),
            ),
            client.V1Volume(
                name="percebro-auth",
                secret=client.V1SecretVolumeSource(
                    secret_name=f"{self.release}-percebro.auth"
                ),
            ),
            client.V1Volume(
                name="models-storage",
                persistent_volume_claim=client.V1PersistentVolumeClaimVolumeSource(
                    claim_name=f"{self.release}-models-pvc"
                ),
            ),
            client.V1Volume(
                name="sample-data-storage",
                persistent_volume_claim=client.V1PersistentVolumeClaimVolumeSource(
                    claim_name=f"{self.release}-sample-data-pvc"
                ),
            ),
            client.V1Volume(
                name="videos-storage",
                persistent_volume_claim=client.V1PersistentVolumeClaimVolumeSource(
                    claim_name=f"{self.release}-videos-pvc"
                ),
            ),
            client.V1Volume(
