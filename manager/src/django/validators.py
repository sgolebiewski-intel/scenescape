...
def verify_image_set(files_list, basefilename):
    ...
    """
    if (
        len(list(filter(lambda v: re.match(f"{basefilename}/keyframes", v), files_list)))
        == 0
    ):
        return False
    images_list = [
        ...
        if basefilename + "/keyframes/images" in file and file.endswith(".jpg")
    ]
    depth_list = [
        file
        for file in files_list
        if basefilename + "/keyframes/depth" in file and file.endswith(".png")
    ]
    camera_json_list = [
        file
        for file in files_list
        if basefilename + "/keyframes/cameras" in file and file.endswith(".json")
    ]
...
def is_polycam_dataset(basefilename, filenames):
    ...
    """
    return (
        basefilename + "/raw.glb" in filenames
        and basefilename + "/mesh_info.json" in filenames
        and verify_image_set(filenames, basefilename)
    )
