...
class ModelDirectory(APIView):
    ...
    def loadDirectory(self, path, folder_name):
        ...
        if not os.path.exists(full_path):
            return (
                "Path not found. It may have been deleted. Please refresh the page.",
        ...
        # Count depth level
        depth_level = len(
            [part for part in os.path.join(path, folder_name).split(os.sep) if part]
        )

        # Render the directory content in html format
        html = render_to_string(
            "model/includes/model_directory.html",
            {
                "path": path,
