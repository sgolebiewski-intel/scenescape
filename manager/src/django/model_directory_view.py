...
class ModelDirectory(APIView):
    ...
    def loadDirectory(self, path, folder_name):
        ...
        full_path, error = self.safePathJoin(
            settings.MODEL_ROOT, path, folder_name)
        if error:
        ...
        if os.path.isfile(full_path):  # File case
            ...
            for dirpath, dirnames, filenames in os.walk(full_path):
                ...
                # Reset the current level to the directory structure to
                # requested folder
        ...
        # Count depth level
        depth_level = len([part for part in os.path.join(
            path, folder_name).split(os.sep) if part])
    ...
    def createDirectory(self, path, new_folder_name):
        ...
        try:
            mkdir, error = self.safePathJoin(
                settings.MODEL_ROOT, path, new_folder_name)
            if error:
                return 'Invalid path', status.HTTP_400_BAD_REQUEST
            ...
            return f'Error creating directory: {
    ...
    def deleteDirectory(self, path, delete_folder_name):
        ...
        delete_path, error = self.safePathJoin(
            settings.MODEL_ROOT, path, delete_folder_name)
        if error:
            return 'Invalid path', status.HTTP_400_BAD_REQUEST
