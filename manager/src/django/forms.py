...
class CamCalibrateForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.kubernetes_fields = [
            'command', 'camerachain'] + self.advanced_fields
        super().__init__(*args, **kwargs)
        if not settings.KUBERNETES_SERVICE_HOST:
            for field in self.kubernetes_fields:
                del self.fields[field]
        self.fields['intrinsics_cx'].widget = forms.TextInput(
            attrs={'readonly': 'readonly'})
        self.fields['intrinsics_cy'].widget = forms.TextInput(
            attrs={'readonly': 'readonly'})
        self.fields['distortion_k2'].widget = forms.TextInput(
            attrs={'readonly': 'readonly'})
        self.fields['distortion_p1'].widget = forms.TextInput(
            attrs={'readonly': 'readonly'})
        self.fields['distortion_p2'].widget = forms.TextInput(
            attrs={'readonly': 'readonly'})
        self.fields['distortion_k3'].widget = forms.TextInput(
            attrs={'readonly': 'readonly'})
    ...
    def clean(self):
        ...
        if new_file and self.instance.polycam_data != new_file:
            ...
            if self.instance.polycam_hash == file_hash:
                self.add_error(
                    'polycam_data',
                    "Uploading a duplicate zip file is not allowed. Please clear the field and upload again.")
            else:
        ...
        if area == "poly":
            if len(rois) < 1:
                raise ValidationError(
                    "Please draw a custom region (polygon) with at least 3 vertices")
            if len(rois[0]["points"]) < 3:
                raise ValidationError(
                    "The custom region (polygon) must have at least 3 vertices")
            for point in rois[0]["points"]:
                try:
                    ...
                    raise ValidationError(
                        "The polygon vertex coordinates must be floating point numbers.")
        return cleaned_data
...
class ChildSceneForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if is_update:
            ...
            self.fields['parent'].queryset = Scene.objects.filter(
                name=self.instance.parent)
            self.fields['child'].queryset = Scene.objects.filter(
                name=self.instance.child)
        else:
        ...
        # Filter out all the Scenes that have a parent and ones that create
    ...
    def _isParentInHierarchy(self, parent, child):
        ...
        while stack:
            ...
            if parent == current_child.child:
                return True
            for childObj in ChildScene.objects.filter(
    ...
    def clean(self):
        ...
        if cleaned_data['child_type'] == 'remote':
            if cleaned_data['child_name'] == cleaned_data['parent'].name:
                self.add_error('child_name',
                               "Parent and child cannot have same name.")
            elif cleaned_data['remote_child_id'] == cleaned_data['parent'].id:
                self.add_error(
                    'remote_child_id',
                    "Parent and child cannot have same id.")
            elif Scene.objects.filter(id=cleaned_data['remote_child_id']).exists():
                self.add_error(
                    'remote_child_id',
                    "Scene with this id already exists. Create a local child scene.")
        return cleaned_data
