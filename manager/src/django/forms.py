...
class SingletonCreateForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {"child_type": forms.RadioSelect(choices=SINGLETON_CHOICES)}
...
class ChildSceneForm(forms.ModelForm):
    class Meta:
        model = ChildScene
        child_types = [("local", "local"), ("remote", "remote")]
        fields = [
            "child_type",
            "child",
            "remote_child_id",
    ...
    def clean(self):
        ...
        if cleaned_data["child_type"] == "remote":
            if cleaned_data["child_name"] == cleaned_data["parent"].name:
                ...
                self.add_error(
                    "remote_child_id", "Parent and child cannot have same id."
                )
