    ...
    if broker is not None:
        ...
        try:
            ...
            if scene_id:
                client.publish(
                    PubSub.formatTopic(
                        PubSub.CMD_SCENE_UPDATE,
                        scene_id=scene_id),
                    "update")
            if camera_data:
                client.publish(
                    PubSub.formatTopic(
                        PubSub.CMD_KUBECLIENT),
...
class Scene(models.Model):
    ...
    map_processed = models.DateTimeField(
        "Last Processed at", null=True, editable=False)
    output_lla = models.BooleanField(
    ...
    def _getDefaultSiftDict():
        return {"sift": dict()}
    local_feature = models.JSONField(
        default=_getDefaultSiftDict, null=True, blank=True)

    def _getDefaultNnRatioDict():
        return {"NN-ratio": dict()}
    matcher = models.JSONField(
        default=_getDefaultNnRatioDict,
        null=True,
        blank=True)
    ...
    def autoAlignSceneMap(self):
        ...
        if self.map and os.path.splitext(self.map.path)[1].lower() == ".glb":
            ...
            self.rotation_y = 0.0
            self.rotation_z = 0.0
            mesh, _ = extractMeshFromGLB(self.map.path, rotation=np.array(
                ...
                with zipfile.ZipFile(glb_from_zip.path, 'r') as zf:
                    ...
                    glb_content = zf.read(
                        os.path.join(
                            base_file_name,
                            "raw.glb"))
                    self.map.save(f"{self.name}.glb", ContentFile(
                        glb_content), save=False)
    ...
    def scenescapeScene(self):
        ...
        if not mScene:
            mScene = ScenescapeScene(
                self.name,
                self.map.path if self.map else None,
                self.scale)
    ...
    def scenescapeSceneUpdateSensors(self, mScene, force=False):
        ...
        for sensor in self.sensor_set.all():
            if sensor.type == "camera" and (
                ...
                if cam.intrinsics_fx is not None and cam.intrinsics_fy is not None \
                   and cam.intrinsics_cx is not None and cam.intrinsics_cy is not None:
                    sInfo['intrinsics'] = [
                        cam.intrinsics_fx,
                        cam.intrinsics_fy,
                        cam.intrinsics_cx,
                ...
                if cam.distortion_k1 is not None and cam.distortion_k2 is not None \
                   and cam.distortion_p1 is not None and cam.distortion_p2 is not None \
...
class ChildScene(models.Model):
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                check=models.Q(
                    child__isnull=False,
                    child_name__isnull=True) | models.Q(
                    child__isnull=True,
                    child_name__isnull=False),
    ...
    transform_type = models.CharField(
        max_length=10,
        choices=CHILD_SCENE_TRANSFORM_CHOICES,
        default=MATRIX)
...
class Cam(Sensor):
    ...
    intrinsics_fx = models.FloatField(
        default=None, null=True, blank=True, validators=[
            MinValueValidator(0.001)])
...
class SingletonSensor(Sensor):
    ...
    def get_sectors(self):
        if not hasattr(self, 'singleton_scalar_threshold'):
            return [{"color": "green", "color_min": "0"}, {
                "color": "yellow", "color_min": "2"}, {"color": "red", "color_min": "5"}], 10
...
class BoundingBox(models.Model):
    ...
    def within(self, coord):
        ...
        line = ScenescapeLine(
            ScenescapePoint(coord), ScenescapePoint(
        ...
        for i in range(len(points)):
            ...
            if isect and line.isPointOnLine(
                    isect) and segment.isPointOnLine(isect):
                crossings += 1
        if crossings & 1 == 0:
            return False
        return True
...
class Asset3D(models.Model):
    ...
    rotation_from_velocity = models.BooleanField(
        choices=BOOLEAN_CHOICES, default=False, null=True)
    tracking_radius = models.FloatField(
        "Tracking radius (meters)", default=2.0)
...
class RegionOccupancyThreshold(models.Model):
    region = models.OneToOneField(
        Region,
        on_delete=models.CASCADE,
        related_name='roi_occupancy_threshold')
