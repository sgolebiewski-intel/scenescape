...
from scene_common.camera import (
    Camera as ScenescapeCamera,
    CameraPose as ScenescapeCameraPose,
)
...
from scene_common.geometry import (
    Region as ScenescapeRegion,
    Tripwire as ScenescapeTripwire,
)
...
def sendUpdateCommand(scene_id=None, camera_data=None):
    ...
    if broker is not None:
        ...
        try:
            ...
            if scene_id:
                client.publish(
                    PubSub.formatTopic(PubSub.CMD_SCENE_UPDATE, scene_id=scene_id),
                    "update",
                )
            if camera_data:
                client.publish(
                    PubSub.formatTopic(PubSub.CMD_KUBECLIENT),
                    json.dumps(camera_data),
                    qos=2,
                )
            msg = client.publish(
                PubSub.formatTopic(PubSub.CMD_DATABASE), "update", qos=1
            )
...
class Scene(models.Model):
    ...
    map = models.FileField(
        "Scene map as .glb or image or .zip",
        default=None,
        null=True,
        blank=True,
        validators=[
            FileExtensionValidator(["glb", "png", "jpeg", "jpg", "zip"]),
            validate_map_file,
        ],
    )
    ...
    rotation_x = models.FloatField(
        "X Rotation (degrees)", default=0.0, null=True, blank=False
    )
    rotation_y = models.FloatField(
        "Y Rotation (degrees)", default=0.0, null=True, blank=False
    )
    rotation_z = models.FloatField(
        "Z Rotation (degrees)", default=0.0, null=True, blank=False
    )
    translation_x = models.FloatField(
        "X Translation (meters)", default=0.0, null=True, blank=False
    )
    translation_y = models.FloatField(
        "Y Translation (meters)", default=0.0, null=True, blank=False
    )
    translation_z = models.FloatField(
        "Z Translation (meters)", default=0.0, null=True, blank=False
    )
    ...
    polycam_data = models.FileField(
        blank=True,
        null=True,
        validators=[FileExtensionValidator(["zip"]), validate_zip_file],
    )
    ...
    global_descriptor_file = models.FileField(
        blank=True,
        null=True,
    ...
    global_feature = models.CharField(
        verbose_name="Global Feature Matching Algorithm",
        max_length=200,
        default="netvlad",
        blank=True,
    )
    ...
    polycam_hash = models.CharField(
        null=True, blank=True, max_length=100, editable=False
    )
    apriltag_size = models.FloatField(
        "AprilTag Size (meters)", max_length=10, default=0.162, null=True, blank=True
    )
    regulated_rate = models.FloatField(
        "Regulate Rate (Hz)",
        default=30,
        blank=True,
        validators=[MinValueValidator(0.001)],
    )
    external_update_rate = models.FloatField(
        "Max External Update Rate (Hz)",
        default=30,
        blank=True,
        validators=[MinValueValidator(0.001)],
    )
    inlier_threshold = models.FloatField(
        "Feature Match Confidence Threshold",
        default=0.5,
        blank=True,
    ...
    def changedCalibrationParams(self):
        return (
            self._original_calibration_mode != self.camera_calibration
            or self._original_matcher != self.matcher
            or self._original_num_of_localization != self.number_of_localizations
            or self._original_global_feature != self.global_feature
            or self._original_local_feature != self.local_feature
            or self._original_min_num_of_matches != self.minimum_number_of_matches
            or self._original_scale != self.scale
            or self._original_apriltag_size != self.apriltag_size
            or self._original_inlier_threshold != self.inlier_threshold
        )

    def regenerateThumbnail(self):
        return (
            self.map != self._original_map
    ...
    def save(self, *args, **kwargs):
        ...
        try:
            ...
            if (self._original_map != self.map) and os.path.splitext(self.map.name)[
                1
            ].lower() == ".zip":
            ...
            if glb_from_zip:
                with zipfile.ZipFile(glb_from_zip.path, "r") as zf:
                    ...
                    self.map.save(
                        f"{self.name}.glb", ContentFile(glb_content), save=False
                    )
            ...
            if self.regenerateThumbnail() or glb_from_zip:
                if not self.map:
                    ...
                    if ext == ".glb":
                        img_data, pixels_per_meter = generateOrthoView(
                            self, self.map.path
                        )
                        ...
                        with ContentFile(b"") as imgfile:
                            ...
                            self.thumbnail.save(
                                self.name + "_2d.png", imgfile, save=False
                            )
                    else:
                        self.thumbnail = None
                        self.resetRotation()
    ...
    def roiJSON(self):
        ...
        for region in self.regions.all():
            ...
            # provide points in the right order, so ROI polygon is formed
            # properly
    ...
    def scenescapeScene(self):
        ...
        if not mScene:
            mScene = ScenescapeScene(
                self.name, self.map.path if self.map else None, self.scale
            )
            mScene.output_lla = self.output_lla
            mScene.mesh_translation = [
                self.translation_x,
                self.translation_y,
                self.translation_z,
            ]
    ...
    def scenescapeSceneUpdateSensors(self, mScene, force=False):
        ...
        for sensor in self.sensor_set.all():
            if sensor.type == "camera" and (
                force or sensor.sensor_id not in mScene.cameras
            ):
                cam = sensor.cam
                ...
                if (
                    cam.intrinsics_fx is not None
                    and cam.intrinsics_fy is not None
                    and cam.intrinsics_cx is not None
                    and cam.intrinsics_cy is not None
                ):
                    sInfo["intrinsics"] = [
                        cam.intrinsics_fx,
                        cam.intrinsics_fy,
                ...
                elif cam.intrinsics_fx is not None:
                    sInfo["intrinsics"] = [cam.intrinsics_fx]
                    if cam.intrinsics_fy is not None:
                        sInfo["intrinsics"].append(cam.intrinsics_fy)

                if (
                    cam.distortion_k1 is not None
                    and cam.distortion_k2 is not None
                    and cam.distortion_p1 is not None
                    and cam.distortion_p2 is not None
                    and cam.distortion_k3 is not None
                ):
                    sInfo["distortion"] = [
                        cam.distortion_k1,
                        cam.distortion_k2,
    ...
    def scenescapeSceneUpdateRegions(self, mScene):
        ...
        for tripwire in self.tripwires.all():
            ...
            if tripwire.name in mScene.tripwires:
                ...
                mScene.tripwires[tripwire.name] = ScenescapeTripwire(
                    tripwire.uuid, tripwire.name, rPoints
                )
    ...
    def wssConnection(self):
        ...
        userpass = (
            urllib.parse.quote(data["user"], safe="")
            + ":"
...
class ChildScene(models.Model):
    ...
    child_name = models.CharField(
        "Child Name", default=None, max_length=200, null=True, blank=True
    )
    remote_child_id = models.UUIDField(
        "Remote Child ID", default=None, null=True, blank=True, unique=True
    )
    parent = models.ForeignKey(
        Scene, null=False, blank=False, on_delete=models.CASCADE, related_name="children"
    )
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                check=models.Q(child__isnull=False, child_name__isnull=True)
                | models.Q(child__isnull=True, child_name__isnull=False),
    ...
    transform_type = models.CharField(
        max_length=10, choices=CHILD_SCENE_TRANSFORM_CHOICES, default=MATRIX
    )
    host_name = models.CharField("Hostname or IP", max_length=200, null=True, blank=True)
    mqtt_username = models.CharField(
        "MQTT Username", max_length=200, null=True, blank=True
    )
    mqtt_password = models.CharField(
        "MQTT Password", max_length=200, null=True, blank=True
    )
    retrack = models.BooleanField(
        "Retrack objects", choices=BOOLEAN_CHOICES, default=True, blank=True
    )

    @property
    def cameraPose(self):
        return ScenescapeCameraPose(
            ScenescapeCameraPose.arrayToDictionary(
                [
                    self.transform1,
                    self.transform2,
                    self.transform3,
...
class Sensor(models.Model):
    sensor_id = models.CharField(
        max_length=20, default=None, unique=True, verbose_name="Sensor ID"
    )
    name = models.CharField(max_length=200, unique=True)
    sensor_type_choices = (("camera", "Camera"), ("generic", "generic"))
    ...
    def areaJSON(self):
        ...
        rdict = {
            "area": self.singletonsensor.area,
...
class Cam(Sensor):
    ...
    command = models.CharField(
        default=None, max_length=512, null=True, verbose_name="Camera (Video Source)"
    )
    ...
    cv_subsystem = models.CharField(
        default=None, max_length=64, null=True, blank=True, verbose_name="CV Subsystem"
    )
    ...
    if settings.DATABASES["default"]["ENGINE"] == "django.db.backends.sqlite3":
        ...
        transforms = ArrayField(
            models.FloatField(blank=True, default=None, null=True),
            blank=True,
            default=list,
        )
    transform_type = models.CharField(
        max_length=26, choices=CAM_TRANSFORM_CHOICES, default=POINT_CORRESPONDENCE
    )
    ...
    def transformation(self):
        return ScenescapeCameraPose.arrayToDictionary(
            self.transforms, self.transform_type
        )
    ...
    def save(self, *args, **kwargs):
        ...
        transaction.on_commit(
            partial(sendUpdateCommand, camera_data=self.cameraData("save"))
        )
        return
    ...
    def delete(self, *args, **kwargs):
        super().delete(*args, **kwargs)
        transaction.on_commit(
            partial(sendUpdateCommand, camera_data=self.cameraData("delete"))
        )
        return
...
class SingletonSensor(Sensor):
    ...
    singleton_type = models.CharField(
        "Type of Sensor",
        max_length=20,
        choices=SINGLETON_CHOICES,
    ...
    def get_sectors(self):
        ...
        return (
            self.singleton_scalar_threshold.sectors,
            self.singleton_scalar_threshold.range_max,
        )
...
class MobileObject(models.Model):
    ...
    def expected(self, ts):
        ...
        if not v:
            v = (0, 0)
        return (
            self.x + v[0] * (ts - self.log.timestamp),
...
class CamLog(models.Model):
    log = models.OneToOneField(
        DataLog, on_delete=models.CASCADE, primary_key=True, related_name="camLog"
    )
...
class Region(BoundingBox):
    ...
    def get_sectors(self):
        ...
        return (
            self.roi_occupancy_threshold.sectors,
            self.roi_occupancy_threshold.range_max,
        )
...
class TripwirePoint(BoundingBoxPoints):
    tripwire = models.ForeignKey(
        Tripwire, on_delete=models.CASCADE, related_name="points"
    )
...
class Asset3D(models.Model):
    ...
    mark_color = models.CharField(
        "Mark Color", max_length=20, default="#888888", blank=True
    )
    model_3d = models.FileField(
        blank=True, null=True, validators=[FileExtensionValidator(["glb"]), validate_glb]
    )
    rotation_x = models.FloatField(
        "X Rotation (degrees)", default=0.0, null=True, blank=True
    )
    rotation_y = models.FloatField(
        "Y Rotation (degrees)", default=0.0, null=True, blank=True
    )
    rotation_z = models.FloatField(
        "Z Rotation (degrees)", default=0.0, null=True, blank=True
    )
    translation_x = models.FloatField(
        "X Translation (meters)", default=0.0, null=True, blank=True
    )
    translation_y = models.FloatField(
        "Y Translation (meters)", default=0.0, null=True, blank=True
    )
    translation_z = models.FloatField(
        "Z Translation (meters)", default=0.0, null=True, blank=True
    )
    scale = models.FloatField("Scale", default=1.0, null=True, blank=True)
    rotation_from_velocity = models.BooleanField(
        choices=BOOLEAN_CHOICES, default=False, null=True
    )
    ...
    project_to_map = models.BooleanField(
        choices=BOOLEAN_CHOICES, default=False, null=True
    )
...
class RegionOccupancyThreshold(models.Model):
    region = models.OneToOneField(
        Region, on_delete=models.CASCADE, related_name="roi_occupancy_threshold"
    )
...
class SingletonScalarThreshold(models.Model):
    singleton = models.OneToOneField(
        SingletonSensor,
        on_delete=models.CASCADE,
        related_name="singleton_scalar_threshold",
    )
    sectors = models.JSONField(default=list)
    range_max = models.IntegerField(default=10)
