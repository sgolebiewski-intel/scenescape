...
def calculate_pose(rvec, tvec):
    ...
    euler = Rotation.from_matrix(y_up_to_y_down(
        T_inv[:3, :3])).as_euler('XYZ', degrees=False)
...
class CalculateCameraIntrinsics(APIView):
    def post(self, request):
        log.info(
            f"Received request to calculate intrinsics with {
                request.data}")
        try:
            if len(
                    request.data['mapPoints']) != len(
                    request.data['camPoints']) or len(
                    request.data['mapPoints']) < 4:
                return Response(
                    {
                        "error": "Invalid number of points provided for calculation."},
            ...
            # FIXME: Consolidate pose calculation with the one in
            # scene_common/transform.py
            ...
            for keys, flag, min_points in calibrate_flags:
                if any(fix_intrinsics.get(key, True)
                       for key in keys) or num_points < min_points:
                    flags |= flag
