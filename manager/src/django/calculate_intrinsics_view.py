...
def calculate_pose(rvec, tvec):
    ...
    euler = Rotation.from_matrix(y_up_to_y_down(T_inv[:3, :3])).as_euler(
        "XYZ", degrees=False
    )
...
class CalculateCameraIntrinsics(APIView):
    def post(self, request):
        ...
        try:
            if (
                len(request.data["mapPoints"]) != len(request.data["camPoints"])
            ...
            for keys, flag, min_points in calibrate_flags:
                if (
                    any(fix_intrinsics.get(key, True) for key in keys)
            ...
            return Response(
                {"euler": euler, "position": position, "mtx": mtx, "dist": dist},
                status=status.HTTP_200_OK,
            )
        except (cv2.error, TypeError, ValueError) as e:
            log.error(f"Error calculating intrinsics: {e}")
            return Response(
                {"error": "Invalid values provided for calculation"},
                status=status.HTTP_400_BAD_REQUEST,
            )
