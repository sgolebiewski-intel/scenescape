    ...
    def validate(self, data):
        ...
        if len(username) > 150 or len(password) > 150:
            raise serializers.ValidationError(
                "Username/Password must be 150 characters or fewer.")
        else:
            user = authenticate(
    ...
    def link_points(instance, points):
        ...
        for idx, point in enumerate(points):
            if isinstance(instance, Tripwire):
                TripwirePoint.objects.create(
                    tripwire=instance, sequence=idx + 1, x=point[0], y=point[1])
            elif isinstance(instance, Region):
                RegionPoint.objects.create(region=instance, sequence=idx + 1,
                                           x=point[0], y=point[1])
            else:
                SingletonAreaPoint.objects.create(
                    singleton=instance, sequence=idx + 1, x=point[0], y=point[1])
        return
...
class RegionOccupancyThresholdSerializer(serializers.ModelSerializer):
    ...
    def linkRegionOccupancyThreshold(instance, color_range):
        ...
        RegionOccupancyThreshold.objects.create(
            region=instance,
            sectors=color_range['sectors'],
...
class SingletonScalarThresholdSerializer(RegionOccupancyThresholdSerializer):
    ...
    def linkSingletonScalarThreshold(instance, color_range):
        ...
        SingletonScalarThreshold.objects.create(
            singleton=instance,
            sectors=color_range['sectors'],
            range_max=color_range['range_max'])
        return
...
class SingletonSerializer(NonNullSerializer):
    ...
    color_ranges = SingletonScalarThresholdSerializer(
        source='singleton_scalar_threshold')
    ...
    def validate(self, data):
        ...
        if 'color_ranges' in data:
            SingletonScalarThresholdSerializer.validateColorRanges(
                data['color_ranges'])
        return data
    ...
    def create_update(self, validated_data, instance=None):
        ...
        if color_ranges:
            SingletonScalarThresholdSerializer.linkSingletonScalarThreshold(
                instance, color_ranges)
...
class CamSerializer(NonNullSerializer):
    ...
    def map_intrinsics_fields(self, validated_data):
        ...
        try:
            ...
            raise serializers.ValidationError(
                {"intrinsics": "invalid intrinsics: \"" + str(intrinsics) + "\""})

        extended_data = {
            'intrinsics_' +
            key: None for key in CameraIntrinsics.INTRINSICS_KEYS}
    ...
    def map_distortion_fields(self, validated_data):
        ...
        try:
            ...
            raise serializers.ValidationError(
                {"distortion": "invalid distortion: \"" + str(distortion) + "\""})

        extended_data = {
            'distortion_' +
            key: None for key in CameraIntrinsics.DISTORTION_KEYS}
    ...
    def map_transform_fields(self, validated_data):
        ...
        if transform_type in (
            EULER,
            QUATERNION) and all(
            key in self.initial_data for key in (
                'translation',
                'rotation',
                'scale')):
            ...
            extended_data['transforms'] = self.initial_data.get(
                'transforms', None)
    ...
    def create_camera_instance(self, instance):
        ...
        instance.scene.scenescapeSceneUpdateSensors(
            instance.scene.scenescapeScene)
        return
    ...
    def get_intrinsics(self, obj):
        ...
        elif cam.intrinsics_fx is not None and cam.intrinsics_fy is not None:
            return {
    ...
    def get_scale(self, obj):
        if not obj.scene:
            return None
        camera = obj.scene.scenescapeScene.cameraWithID(obj.sensor_id)
        return camera.pose.scale if camera and hasattr(
            camera, 'pose') else None
...
class RegionSerializer(NonNullSerializer):
    ...
    color_ranges = RegionOccupancyThresholdSerializer(
        source='roi_occupancy_threshold')
    ...
    def create_update(self, validated_data, instance=None):
        ...
        if color_ranges:
            RegionOccupancyThresholdSerializer.validateColorRanges(
                color_ranges)
        ...
        if color_ranges:
            RegionOccupancyThresholdSerializer.linkRegionOccupancyThreshold(
                instance, color_ranges)
...
class SceneSerializer(NonNullSerializer):
    ...
    def get_sensors(self, obj):
        queryset = [
            SingletonSensor.objects.get(
                pk=x.id) for x in obj.sensor_set.all() if x.type == "generic"]
    ...
    def get_rotation(self, obj):
        return [
            obj.rotation_x,
            obj.rotation_y,
            obj.rotation_z] if obj.rotation_x else [
            0,
            0,
            0]
    ...
    def get_translation(self, obj):
        return [
            obj.translation_x,
            obj.translation_y,
            obj.translation_z] if obj.translation_x else [
            0,
            0,
            0]
    ...
    def get_scale(self, obj):
        return [
            obj.scale_x,
            obj.scale_y,
            obj.scale_z] if obj.scale_x else [
            1,
            1,
            1]
    ...
    def get_children(self, obj):
        ...
        serialized_children = SceneSerializer(
            need_to_serialize, many=True).data
        return children + serialized_children

    @staticmethod
    def check_circular_dependency(parent_scene, child_scene):
        # Check if any of the descendants of the child scene are the parent
        # before linking
        ...
        for child in child_scene_children:
            ...
            while stack:
                ...
                if parent_scene == current_child.child:
                    raise serializers.ValidationError(
                        f"Cannot link \"{parent_scene}\" with " f"\"{child_scene}\" due to circular dependency")
                for child_node in ChildScene.objects.filter(
                        parent=current_child.child):
    ...
    def link_parent(self, parent_uid, child_scene):
        ...
        child_link = ChildScene.objects.create(
            parent=parent_scene, child=child_scene)
        return child_link
        ...
        for trans_type in types:
            ...
            if popped_data:
                for axis, index in axes.items():
                    validated_data[trans_type + '_' +
                                   axis] = popped_data[index]
        return
    ...
    def validate(self, data):
        ...
        if child_type == 'remote' and remote_child_id and parent:
            if remote_child_id == parent:
                raise serializers.ValidationError(
                    {'remote_child_id': 'remote_child_id cannot be the same as parent.'})
            query_child = ChildScene.objects.filter(
                remote_child_id=remote_child_id, parent=parent)
            if self.instance:
                query_child = query_child.exclude(pk=self.instance.pk)
            if query_child.exists():
                raise serializers.ValidationError(
                    {'remote_child_id': f"{remote_child_id} already exists for this parent."})

        if child_type == 'local' and child and parent:
            if child == parent:
                raise serializers.ValidationError(
                    {'child': 'child cannot be the same as parent.'})
            query_child = ChildScene.objects.filter(child=child, parent=parent)
            if self.instance:
                query_child = query_child.exclude(pk=self.instance.pk)
            if query_child.exists():
                raise serializers.ValidationError(
                    {'child': f"{child} already exists for this parent."})
    ...
    def create_update(self, validated_data, instance=None):
        ...
        if is_update and instance.child_type != child_type:
            if child_type == "remote":
                ...
                if instance.child:
                    SceneSerializer.check_circular_dependency(
                        parent_scene, instance.child)
            return super().update(instance, validated_data)

        if child_type == "local" and validated_data.get('child'):
            SceneSerializer.check_circular_dependency(
                parent_scene, validated_data['child'])
