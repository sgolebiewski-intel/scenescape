        ...
        return OrderedDict(
            [
                ...
                if (
                    ...
                    and not isinstance(result[key], tuple)
                )
                or result[key]
            ]
        )
...
class CenterSerializerField(serializers.DictField):
    def to_representation(self, obj):
        if (
            hasattr(obj, "map_x")
            and hasattr(obj, "map_y")
            and obj.map_x is not None
            and obj.map_y is not None
        ):
    ...
    def link_points(instance, points):
        ...
        for idx, point in enumerate(points):
            if isinstance(instance, Tripwire):
                ...
                RegionPoint.objects.create(
                    region=instance, sequence=idx + 1, x=point[0], y=point[1]
                )
            else:
...
class CamSerializer(NonNullSerializer):
    ...
    def map_intrinsics_fields(self, validated_data):
        ...
        extended_data = {
            "intrinsics_" + key: None for key in CameraIntrinsics.INTRINSICS_KEYS
        }
    ...
    def map_distortion_fields(self, validated_data):
        ...
        extended_data = {
            "distortion_" + key: None for key in CameraIntrinsics.DISTORTION_KEYS
        }
    ...
    def map_transform_fields(self, validated_data):
        ...
        if transform_type in (EULER, QUATERNION) and all(
            key in self.initial_data for key in ("translation", "rotation", "scale")
        ):
            # Convert quaternion to euler
            if transform_type == QUATERNION:
                self.initial_data["rotation"] = (
                    Rotation.from_quat(self.initial_data["rotation"])
    ...
    def get_translation(self, obj):
        ...
        return (
            camera.pose.translation.asNumpyCartesian.tolist()
    ...
    def get_rotation(self, obj):
        ...
        camera = obj.scene.scenescapeScene.cameraWithID(obj.sensor_id)
        return (
            getattr(camera.pose, "euler_rotation", None)
            if camera and hasattr(camera, "pose")
            else None
        )
...
class SceneSerializer(NonNullSerializer):
    ...
    def get_sensors(self, obj):
        queryset = [
            SingletonSensor.objects.get(pk=x.id)
            for x in obj.sensor_set.all()
            if x.type == "generic"
        ]
    ...
    def get_translation(self, obj):
        return (
            [obj.translation_x, obj.translation_y, obj.translation_z]
            if obj.translation_x
            else [0, 0, 0]
        )
        ...
        for child in child_scene_children:
            ...
            while stack:
                ...
                if parent_scene == current_child.child:
                    raise serializers.ValidationError(
                        f'Cannot link "{parent_scene}" with '
