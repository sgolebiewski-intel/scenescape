...
template <typename T> class HungarianOptimizer
{
...
template <typename T> void HungarianOptimizer<T>::Maximize(std::vector<std::pair<size_t, size_t>> *assignments)
{
  OptimizationInit();
...
template <typename T> void HungarianOptimizer<T>::Minimize(std::vector<std::pair<size_t, size_t>> *assignments)
{
  OptimizationInit();
  DoMunkres();
...
template <typename T> void HungarianOptimizer<T>::OptimizationInit()
{
...
template <typename T> void HungarianOptimizer<T>::OptimizationClear()
{
...
template <typename T> void HungarianOptimizer<T>::FindAssignments(std::vector<std::pair<size_t, size_t>> *assignments)
{
  assignments->clear();
  for (size_t row = 0; row < height_; ++row)
  {
...
/* Find a column in row 'row' containing a star, or return
 * kHungarianOptimizerColNotFound if no such column exists. */
template <typename T> int HungarianOptimizer<T>::FindStarInRow(const size_t row) const
{
  for (size_t col = 0; col < matrix_size_; ++col)
  {
    if (IsStarred(row, col))
...
/* Find a row in column 'col' containing a star, or return
 * kHungarianOptimizerRowNotFound if no such row exists. */
template <typename T> int HungarianOptimizer<T>::FindStarInCol(const size_t col) const
{
...
/* Find a column in row containing a prime, or return
 * kHungarianOptimizerColNotFound if no such column exists. */
template <typename T> int HungarianOptimizer<T>::FindPrimeInRow(const size_t row) const
{
  for (size_t col = 0; col < matrix_size_; ++col)
  {
    if (IsPrimed(row, col))
...
template <typename T> void HungarianOptimizer<T>::ClearPrimes()
{
  for (size_t row = 0; row < matrix_size_; ++row)
  {
    for (size_t col = 0; col < matrix_size_; ++col)
...
template <typename T> void HungarianOptimizer<T>::ClearCovers()
{
...
/* Find the smallest uncovered cell in the matrix. */
template <typename T> T HungarianOptimizer<T>::FindSmallestUncovered()
{
 ...
 * and return true, or return false if no such cell exists. */
template <typename T> bool HungarianOptimizer<T>::FindZero(size_t *zero_row, size_t *zero_col)
{
  uncov_col_.clear();
  uncov_row_.clear();
...
/* Print the matrix to stdout (for debugging). */
template <typename T> void HungarianOptimizer<T>::PrintMatrix()
{
  for (size_t row = 0; row < matrix_size_; ++row)
  {
    for (size_t col = 0; col < matrix_size_; ++col)
...
/* Run the Munkres algorithm */
template <typename T> void HungarianOptimizer<T>::DoMunkres()
{
...
template <typename T> void HungarianOptimizer<T>::CheckStar()
{
  for (size_t row = 0; row < height_; ++row)
  {
    int star_col = -1;
...
template <typename T> void HungarianOptimizer<T>::ReduceRows()
{
  for (size_t row = 0; row < matrix_size_; ++row)
  {
 ...
 * Step3. */
template <typename T> void HungarianOptimizer<T>::StarZeroes()
{
...
template <typename T> void HungarianOptimizer<T>::CoverStarredZeroes()
{
 ...
 * and uncover the column containing the starred zero. Continue in this manner
 * until there are no uncovered zeros left, then go to Step 6. */
template <typename T> void HungarianOptimizer<T>::PrimeZeroes()
{
 ...
 * series, erase all primes and uncover every line in the matrix. Return to
 * Step 3. */
template <typename T> void HungarianOptimizer<T>::MakeAugmentingPath()
{
  bool done = false;
  size_t count = 0;
 ...
 * covered row, and subtract it from every element of each uncovered column.
 * Return to Step 4 without altering any stars, primes, or covered lines. */
template <typename T> void HungarianOptimizer<T>::AugmentPath()
{
