...
# This example shows how a custom tracker can be created using the
# components from robot_vision.tracking
...
class CustomTracker:
    def __init__(
        self,
        track_manager_config: tracking.TrackManagerConfig = None,
        distance_type: tracking.DistanceType = None,
        distance_threshold: float = 1.0,
    ):
        """
        Instantiate the custom tracker with given config and distance parameters
        """
        # For the custom tracker we will use the same match function and
        # track_manager defined in robot vision
        self.distance_type = (
            tracking.DistanceType.Euclidean if distance_type is None else distance_type
        )
        self.distance_threshold = distance_threshold
        self.track_manager_config = (
            tracking.TrackManagerConfig()
            if track_manager_config is None
            else track_manager_config
        )
    ...
    def match_function(self, tracks, objects):
        ...
        return tracking.match(
            tracks, objects, self.distance_type, self.distance_threshold
        )
    ...
    def _update(
        self, objects: List[tracking.TrackedObject], timestamp: datetime.datetime
    ):
        """
        ...
        assignments, unassigned_tracks, unassigned_objects = self.match_function(
            tracks, objects
        )

        for track_index, object_index in assignments:
            self.track_manager.set_measurement(
                tracks[track_index].id, objects[object_index]
            )
...
print(
    f"Simulating an object starting at location ({x0}, {y0}) and moving with velocity ({vx}, {vy}) for {total_time} seconds."
)
...
for t in np.arange(step, total_time + 1e-3, step):  # initial time is step
    ...
    object_ = create_object_at_location(
        x=x, y=y, classification=classification_data.classification("class1", 0.6)
    )
...
ground_truth_object = create_object_at_location(
    x=x0 + vx * t,
    y=y0 + vy * t,
    classification=classification_data.classification("class1", 1.0),
)
