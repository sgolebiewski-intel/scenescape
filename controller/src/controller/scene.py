...
class Scene(SceneModel):
    ...
    def _createMovingObjectsForDetection(
            self, detectionType, detections, when, camera):
        objects = []
    ...
    def processCameraData(self, jdata, when=None, ignoreTimeFlag=False):
        ...
        if camera_id in self.cameras:
            ...
            if 'frame_rate' in jdata:
                self.ref_camera_frame_rate = min(
                    jdata['frame_rate'],
        ...
        for info in new:
            if 'lat_long_alt' in info:
                if 'translation' in info:
                    ...
                    return True
                info['translation'] = convertLLAToECEF(
                    info.pop('lat_long_alt'))
    ...
    def processSensorData(self, jdata, when):
        ...
        if hasattr(
                sensor,
                'lastWhen') and sensor.lastWhen is not None and when <= sensor.lastWhen:
    ...
    def updateRegionEvents(self, detectionType, regions, now, now_str):
        ...
        for key in regions:
            ...
            if hasattr(
                    region,
                    'value') and region.singleton_type == "environmental":
                for obj in newObjects:
        ...
        for obj in curObjects:
            ...
            for sname in self.cameras:
                ...
                if hasattr(
                        ...
                        'pose') and hasattr(
                        camera.pose,
                        'regionOfView') and camera.pose.regionOfView.isPointWithin(
                        obj.sceneLoc):
    ...
    def computePixelsToMeterPlane(
        ...
        pt = cv2.undistortPoints(
            ...
            cameraintrinsicsmatrix,
            distortionmatrix)
        oppositepxpoint = np.array(
            [x + width, y + height], dtype='float64').reshape(-1, 1, 2)
        opppt = cv2.undistortPoints(
            oppositepxpoint,
            cameraintrinsicsmatrix,
            distortionmatrix)
        return pt[0][0][0], pt[0][0][1], opppt[0][0][0] - \
            pt[0][0][0], opppt[0][0][1] - pt[0][0][1]
