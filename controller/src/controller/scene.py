...
class Scene(SceneModel):
    ...
    def updateTracker(
        ...
        # Only update tracker if the values have changed to avoid losing
        # tracking data
        if (
            max_unreliable_time != self.max_unreliable_time
            or non_measurement_time_dynamic != self.non_measurement_time_dynamic
            or non_measurement_time_static != self.non_measurement_time_static
        ):
            self.max_unreliable_time = max_unreliable_time
            self.non_measurement_time_dynamic = non_measurement_time_dynamic
            self.non_measurement_time_static = non_measurement_time_static
    ...
    def processCameraData(self, jdata, when=None, ignoreTimeFlag=False):
        ...
        if camera_id in self.cameras:
            ...
            if "frame_rate" in jdata:
                self.ref_camera_frame_rate = (
                    min(jdata["frame_rate"], self.ref_camera_frame_rate)
                    if self.ref_camera_frame_rate is not None
                    else jdata["frame_rate"]
                )
        else:
        ...
        for detection_type, detections in jdata["objects"].items():
            if "intrinsics" not in jdata:
                # if no intrinsics are provided, then bounding boxes are in
                # pixels and not normalized
                for obj in detections:
                    if "bounding_box" not in obj and "bounding_box_px" in obj:
                        ...
                        agnosticx, agnosticy, agnosticw, agnostich = (
                            self.computePixelsToMeterPlane(
                                x,
                                y,
                                w,
                                h,
                                camera.pose.intrinsics.intrinsics,
                                camera.pose.intrinsics.distortion,
                            )
                        )
                        obj["bounding_box"] = {
                            "x": agnosticx,
            ...
            objects = self._createMovingObjectsForDetection(
                detection_type, detections, when, camera
            )
            self.finishProcessing(detection_type, when, objects)
        return True

    def processSceneData(
        self, jdata, child, cameraPose, detectionType, when=None, ignoreTimeFlag=False
    ):
        new = jdata["objects"]
        ...
        for info in new:
            if "lat_long_alt" in info:
                if "translation" in info:
                    log.warn(
                        "Input data must have only one of 'lat_long_alt' and 'translation'"
                    )
                    return True
            ...
            # Remove reid vector from the object info as tracker does not
            # support reid from scene hierarchy
            if "reid" in info:
                info.pop("reid")

            mobj = self.tracker.createObject(detectionType, info, when, child)
            log.debug("RX SCENE OBJECT", "id=%s" % (mobj.oid), mobj.sceneLoc)
            if child.retrack:
                objects.append(mobj)
            else:
    ...
    def processSensorData(self, jdata, when):
        ...
        if (
            hasattr(sensor, "lastWhen")
            and sensor.lastWhen is not None
            and when <= sensor.lastWhen
        ):
    ...
    def updateTripwireEvents(self, detectionType, now):
        for key in self.tripwires:
            ...
            for obj in curObjects:
                age = now - obj.when
                if obj.frameCount > 3 and len(obj.chain_data.publishedLocations) > 1:
                    d = tripwire.lineCrosses(
                        Line(
                            obj.chain_data.publishedLocations[0].as2Dxy,
            ...
            if (
                len(tripwireObjects) != len(objects)
    ...
    def updateRegionEvents(self, detectionType, regions, now, now_str):
        ...
        for key in regions:
            ...
            curObjects = self.tracker.currentObjects(detectionType)
            for obj in curObjects:
                if obj.frameCount > 3 and region.isPointWithin(obj.sceneLoc):
                    objects.append(obj)
                ...
                for obj in regionObjects:
                    if obj.gid in old:
                        if key in obj.chain_data.regions:
                            entered = get_epoch_time(
                                obj.chain_data.regions[key]["entered"]
                            )
                            dwell = now - entered
    ...
    def updateVisible(self, curObjects):
        ...
        for obj in curObjects:
            ...
            for sname in self.cameras:
                ...
                if (
                    hasattr(camera, "pose")
                    and hasattr(camera.pose, "regionOfView")
    ...
    def deserialize(cls, data):
        ...
        scene = cls(
            data["name"], data.get("map", None), data.get("scale", None), *tracker_config
        )
        scene.uid = data["uid"]
    ...
    def updateCameras(self, newCameras):
        ...
        for cameraData in newCameras:
            ...
            self.cameras[camID] = Camera(
                camID, cameraData, resolution=cameraData["resolution"]
            )
        deleted = old - new
        ...
        for regionData in newRegions:
            ...
            if region_uuid in existingRegions:
                ...
                existingRegions[region_uuid] = Region(
                    region_uuid, region_name, regionData
                )
        deleted = old - new
    ...
    def updateTripwires(self, newTripwires):
        ...
        for tripwireData in newTripwires:
            ...
            self.tripwires[tripwire_uuid] = Tripwire(
                tripwire_uuid, tripwire_name, tripwireData
            )
        deleted = old - new
    ...
    def computePixelsToMeterPlane(
        self, x, y, width, height, cameraintrinsicsmatrix, distortionmatrix
    ):
        """
        ...
        oppositepxpoint = np.array([x + width, y + height], dtype="float64").reshape(
            -1, 1, 2
        )
        opppt = cv2.undistortPoints(
            oppositepxpoint, cameraintrinsicsmatrix, distortionmatrix
        )
        return (
            pt[0][0][0],
            pt[0][0][1],
            opppt[0][0][0] - pt[0][0][0],
            opppt[0][0][1] - pt[0][0][1],
        )
