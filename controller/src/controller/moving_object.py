...
class MovingObject:
    ...
    def __init__(self, info, when, camera):
        ...
        if 'bounding_box_px' in self.info:
            ...
            if 'bounding_box' not in self.info:
                agnostic = self.camera.pose.intrinsics.infer3DCoordsFrom2DDetection(
                    self.boundingBoxPixels)
    ...
    def setGID(self, gid):
        self.chain_data = ChainData(
            regions={}, publishedLocations=[], sensors={})
        self.gid = gid
    ...
    def setPrevious(self, otherObj):
        ...
        self.location = [self.location[0]] + \
            otherObj.location[:LOCATION_LIMIT - 1]
    ...
    def inferRotationFromVelocity(self):
        if self.rotation_from_velocity and self.velocity:
            ...
            if speed > SPEED_THRESHOLD:
                ...
                self.rotation = rotationToTarget(
                    direction, velocity).as_quat().tolist()
    ...
    def camLoc(self):
        ...
        if self.shift_type == TYPE_2:
            if not hasattr(self, 'baseAngle'):
                self._projectBounds()
            return Point(bounds.x + bounds.width / 2, bounds.y + \
                         bounds.height - (bounds.height / 2) * (self.baseAngle / 90))
        else:
            ...
            if camera and hasattr(camera, 'pose'):
                if 'rotation' in info:
                    if self.project_to_map:
                        info['translation'], info['rotation'] = camera.pose.projectToMap(
                            info['translation'], info['rotation'], self.map_triangle_mesh.clone(), o3d.core.Tensor(
                                self.map_translation, dtype=o3d.core.Dtype.Float32), o3d.geometry.get_rotation_matrix_from_xyz(
                                self.map_rotation))
                    rotation_as_matrix = Rotation.from_quat(
                        np.array(info['rotation'])).as_matrix()
                ...
                self.orig_point = camera.pose.cameraPointToWorldPoint(
                    Point(info['translation']))
        else:
            if camera and hasattr(camera, 'pose'):
                self.orig_point = camera.pose.cameraPointToWorldPoint(
                    self.camLoc)
                if not self.camLoc.is3D:
                    ...
                    line2 = Line(self.orig_point, Point(np.mean(
                        [self.size[0], self.size[1]]) / 2, line1.angle, 0, polar=True), relative=True)
    ...
    def sceneLoc(self):
        ...
        if not hasattr(self, 'location') or not self.location:
            self._projectBounds()
            self.mapObjectDetectionToWorld(
                self.info, self.first_seen, self.camera)
    ...
    # Below section is for methods that support native tracker or tracker
    # debugger
    def displayIntersections(self, img, ms, pad):
        ...
        for org in self.vectors:
            ...
            point = Point((pt1.x + (pt2.x - pt1.x) / 2,
                          pt1.y + (pt2.y - pt1.y) / 2))
            label = "%i %0.3f,%0.3f" % (self.gid, org.point.x, org.point.y)
            cv2.putText(img, label, point.cv,
    ...
    def load(self, info, scene):
        ...
        self.location = [
            Chronoloc(
                Point(
                    v['point']), v['timestamp'], Rectangle(
                    v['bounding_box'])) for v in info['location']]
        self.vectors = [Vector(scene.cameras[v['camera']], Point(
            v['point']), v['timestamp']) for v in info['vectors']]
        if 'intersected' in info:
            self.intersected = info['intersected']
            if self.intersected:
                self.adjusted = [
                    info['adjusted']['gid'], Point(
                        info['adjusted']['point'])]
                if not self.adjusted[1].is3D:
                    self.adjusted[1] = Point(
                        self.adjusted[1].x, self.adjusted[1].y, DEFAULTZ)
        return
...
class ATagObject(MovingObject):
    def __init__(self, info, when, sensor):
        ...
        self.tag_id = "%s-%s-%s" % (info['category'],
                                    info['tag_family'], info['tag_id'])
        return
    ...
    def mapObjectDetectionToWorld(self, info, when, sensor):
        ...
        # Scale the triangle down to a Z of hover_dist to find point above
        # floor
        pt = sensor.pose.translation - self.orig_point
        if not pt.z == 0:
            pt = Point(
                hover_dist * pt.x / pt.z,
                hover_dist * pt.y / pt.z,
                hover_dist * pt.z / pt.z)
