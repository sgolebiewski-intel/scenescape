...
class Vector:
    ...
    def __repr__(self):
        ...
        return "Vector: %s %s %s" % (origin, str(self.point.log), self.last_seen)
...
class MovingObject:
                ...
                if "rotation" in info:
                    if self.project_to_map:
                        info["translation"], info["rotation"] = camera.pose.projectToMap(
                            ...
                            o3d.core.Tensor(
                                self.map_translation, dtype=o3d.core.Dtype.Float32
                            ),
                            o3d.geometry.get_rotation_matrix_from_xyz(self.map_rotation),
                        )
                    rotation_as_matrix = Rotation.from_quat(
                        np.array(info["rotation"])
                    ).as_matrix()
                    info["rotation"] = list(
                        Rotation.from_matrix(
                            np.matmul(camera.pose.pose_mat[:3, :3], rotation_as_matrix)
                        ).as_quat()
                    )
            ...
            if camera and hasattr(camera, "pose"):
                ...
                if not self.camLoc.is3D:
                    ...
                    line2 = Line(
                        self.orig_point,
                        Point(
                            np.mean([self.size[0], self.size[1]]) / 2,
                            line1.angle,
                            0,
                            polar=True,
                        ),
    ...
    def displayIntersections(self, img, ms, pad):
        ...
        for org in self.vectors:
            ...
            cv2.putText(
                img, label, point.cv, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 3
            )
            cv2.putText(
                img, label, point.cv, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1
            )
        return
            ...
            "location": [
                {
                    ...
                    "timestamp": v.when,
                    "bounding_box": v.bounds.asDict,
                }
                for v in self.location
            ],
            "vectors": [
                {
                    "camera": v.camera.cameraID,
                    "point": (v.point.x, v.point.y, v.point.z),
    ...
    def load(self, info, scene):
        ...
        self.location = [
            Chronoloc(Point(v["point"]), v["timestamp"], Rectangle(v["bounding_box"]))
            for v in info["location"]
        ]
        self.vectors = [
            Vector(scene.cameras[v["camera"]], Point(v["point"]), v["timestamp"])
