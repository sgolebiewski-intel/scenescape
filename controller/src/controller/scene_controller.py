...
from controller.detections_builder import (
    buildDetectionsDict,
    buildDetectionsList,
    computeCameraBounds,
)
...
class SceneController:
    ...
    def __init__(
        ...
        self.pubsub = PubSub(
            mqtt_auth, client_cert, root_cert, mqtt_broker, keepalive=60
        )
    ...
    def publishSceneDetections(self, scene, objects, otype, jdata):
        ...
        if olen > 0 or cid not in scene.lastPubCount or scene.lastPubCount[cid] > 0:
            if "debug_hmo_start_time" in jdata:
                jdata["debug_hmo_processing_time"] = (
                    get_epoch_time() - jdata["debug_hmo_start_time"]
                )
    ...
    def publishExternalDetections(self, scene, otype, jstr):
        ...
        if self.shouldPublish(
            scene.last_published_detection[otype], now, 1 / scene.external_update_rate
        ):
    ...
    def publishRegulatedDetections(
        self, scene_obj, msg_objects, otype, jdata, camera_id
    ):
        ...
        for event_type in scene.events:
            for _, region in scene.events[event_type]:
                ...
                if isinstance(region, Tripwire):
                    ...
                    etype = "region"
                    metadata = region.serialize()
                    metadata["fromSensor"] = region.singleton_type is not None
    ...
    def _buildExitedObjsList(self, scene, region, event_data):
        ...
        for exited_list in exited.values():
            ...
            exited_data = [
                {"object": exited_obj, "dwell": exited_dict[exited_obj["id"]]}
    ...
    def handleMovingObjectMessage(self, client, userdata, message):
        ...
        now = get_epoch_time()
        self.time_offset, self.last_time_sync = adjust_time(
            now,
            self.ntp_server,
            self.ntp_client,
            self.last_time_sync,
        ...
        for detection_type in detection_types:
            ...
            self.publishDetections(
                scene,
                scene.tracker.currentObjects(detection_type),
                msg_when,
                detection_type,
                jdata,
                camera_id,
    ...
    def _handleChildSceneObject(self, sender_id, jdata, detection_type, msg_when):
        ...
        success = scene.processSceneData(
            jdata, sender, sender.cameraPose, detection_type, when=msg_when
        )
    ...
    def republishEvents(self, client, userdata, message):
        ...
        if "from_child_scene" not in msg["metadata"]:
            ...
            msg["metadata"]["from_child_scene"] = (
                sender.name + " > " + msg["metadata"]["from_child_scene"]
            )
        ...
        for k in keys:
            if k == "exited":
                for i, obj in enumerate(event[k]):
                    event[k][i]["object"]["translation"] = (
                        sender.cameraPose.cameraPointToWorldPoint(
                            Point(obj["object"]["translation"])
                        ).asNumpyCartesian.tolist()
                    )
            else:
                for i, obj in enumerate(event[k]):
                    event[k][i]["translation"] = (
                        sender.cameraPose.cameraPointToWorldPoint(
                            Point(obj["translation"])
                        ).asNumpyCartesian.tolist()
                    )
        return
    ...
    def updateSubscriptions(self):
        ...
        for scene in self.scenes:
            for camera in scene.cameras:
                need_subscribe.add(
                    (
                        PubSub.formatTopic(PubSub.DATA_CAMERA, camera_id=camera),
            ...
            if hasattr(scene, "children"):
                ...
                for info in child_scenes.get("results", []):
                    if info["child_type"] == "local":
                        self.cache_manager.sceneWithID(info["child"]).retrack = info[
                            "retrack"
                        ]
                        ...
                        self.cache_manager.cached_child_transforms_by_uid[
                            info["remote_child_id"]
                        ] = Scene.deserialize(info)
                        need_subscribe_child[info["remote_child_id"]] = child_obj
                        need_subscribe.add(
                            (
                                PubSub.formatTopic(
                                    PubSub.SYS_CHILDSCENE_STATUS, scene_name=info["name"]
                                ),
