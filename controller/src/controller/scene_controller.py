...
class SceneController:
    ...
    def __init__(
        ...
        self.cache_manager = CacheManager(
            rest_url, rest_auth, root_cert, self.tracker_config_data)

        self.visibility_topic = visibility_topic
        log.info(
            f"Publishing camera visibility info on ${
                self.visibility_topic} topic.")
        return

    def extractTrackerConfigData(self, tracker_config_file):
        if not os.path.exists(tracker_config_file) and not os.path.isabs(
                tracker_config_file):
            script = os.path.realpath(__file__)
            tracker_config_file = os.path.join(
                os.path.dirname(script), tracker_config_file)
        with open(tracker_config_file) as json_file:
            tracker_config = json.load(json_file)
            self.tracker_config_data["max_unreliable_time"] = tracker_config["max_unreliable_frames"] / \
                tracker_config["baseline_frame_rate"]
            self.tracker_config_data["non_measurement_time_dynamic"] = tracker_config[
                "non_measurement_frames_dynamic"] / tracker_config["baseline_frame_rate"]
            self.tracker_config_data["non_measurement_time_static"] = tracker_config[
                "non_measurement_frames_static"] / tracker_config["baseline_frame_rate"]
        return
    ...
    def publishDetections(self, scene, objects, ts, otype, jdata, camera_id):
        ...
        self.publishRegulatedDetections(
            scene, objects, otype, jdata, camera_id)
    ...
    def publishSceneDetections(self, scene, objects, otype, jdata):
        jdata['objects'] = buildDetectionsList(
            objects, scene, self.visibility_topic == 'unregulated')
        ...
        if olen > 0 or cid not in scene.lastPubCount or scene.lastPubCount[cid] > 0:
            if 'debug_hmo_start_time' in jdata:
                jdata['debug_hmo_processing_time'] = get_epoch_time() - \
                    jdata['debug_hmo_start_time']
            jstr = json.dumps(jdata)
            new_topic = PubSub.formatTopic(
                PubSub.DATA_SCENE, scene_id=scene.uid, thing_type=otype)
    ...
    def publishExternalDetections(self, scene, otype, jstr):
        ...
        if self.shouldPublish(
                scene.last_published_detection[otype],
                now,
                1 / scene.external_update_rate):
            scene.last_published_detection[otype] = get_epoch_time()
            scene_hierarchy_topic = PubSub.formatTopic(
                PubSub.DATA_EXTERNAL, scene_id=scene.uid, thing_type=otype)
            self.pubsub.publish(scene_hierarchy_topic, jstr)
        return

    def publishRegulatedDetections(
            self,
            scene_obj,
            msg_objects,
            otype,
        ...
        if self.shouldPublish(
            ...
            for key in scene['objects']:
                for obj in scene['objects'][key]:
                    if self.visibility_topic == 'regulated':
                        aobj = next(
                            (x for x in msg_objects if x.gid == obj['id']), None)
            ...
            topic = PubSub.formatTopic(
                PubSub.DATA_REGULATED, scene_id=scene_uid)
        ...
        for event_type in scene.events:
            for _, region in scene.events[event_type]:
                ...
                if isinstance(region, Tripwire):
                    ...
                    metadata = region.serialize()
                    metadata['fromSensor'] = (
                        region.singleton_type is not None)
                ...
                detections_dict, num_objects = self._buildAllRegionObjsList(
                    scene, region, event_data)
    ...
    def _buildExitedObjsList(self, scene, region, event_data):
        ...
        for exited_list in exited.values():
            ...
            exited_data = [{'object': exited_obj, 'dwell': exited_dict[exited_obj['id']]}
                           for exited_obj in exited_objs]
    ...
    def handleSensorMessage(self, client, userdata, message):
        ...
        if not self.schema_val.validateMessage(
                "singleton", jdata, check_format=True):
            return
    ...
    def handleMovingObjectMessage(self, client, userdata, message):
        ...
        if 'camera_id' in topic and not self.schema_val.validateMessage(
                "detector", jdata):
            return
        ...
        if lag > self.max_lag:
            if not self.rewrite_bad_time:
                log.warn(
                    "{} FELL BEHIND by {}. SKIPPING {}".format(
                        message.topic, lag, jdata['id']))
                return
            msg_when = now
        ...
        if topic['_topic_id'] == PubSub.DATA_EXTERNAL:
            ...
            success, scene = self._handleChildSceneObject(
                sender_id, jdata, detection_types[0], msg_when)
        else:
        ...
        for detection_type in detection_types:
            jdata['unique_detection_count'] = scene.tracker.getUniqueIDCount(
                detection_type)
            self.publishDetections(scene, scene.tracker.currentObjects(
                detection_type), msg_when, detection_type, jdata, camera_id)
            self.publishEvents(scene, jdata['timestamp'])
        return

    def _handleChildSceneObject(
            self,
            sender_id,
            jdata,
            detection_type,
        ...
        if sender is None:
            remote_sender = self.cache_manager.sceneWithRemoteChildID(
                sender_id)
    ...
    def onConnect(self, client, userdata, flags, rc):
        ...
        # FIXME - update subscriptions when scenes/sensors/children
    ...
    def republishEvents(self, client, userdata, message):
        ...
        msg['metadata'] = applyChildTransform(
            msg['metadata'], sender.cameraPose)
        if 'from_child_scene' not in msg['metadata']:
            msg['metadata']['from_child_scene'] = sender.name
        else:
            msg['metadata']['from_child_scene'] = sender.name + \
                " > " + msg['metadata']['from_child_scene']
            ...
            if hasattr(scene, 'children'):
                ...
                for info in child_scenes.get('results', []):
                    if info['child_type'] == 'local':
                        self.cache_manager.sceneWithID(
                            info['child']).retrack = info['retrack']
                        ...
                        child_obj = ChildSceneController(
                            self.root_cert, info, self)
                        self.cache_manager.cached_child_transforms_by_uid[info['remote_child_id']] = Scene.deserialize(
                            info)
                        need_subscribe_child[info['remote_child_id']
                                             ] = child_obj
                        need_subscribe.add(
                            (PubSub.formatTopic(
