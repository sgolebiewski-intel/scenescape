...
def main():
    ...
    while True:
        ...
        for detectedCategory in objects.keys():
            if len(objects) and objects[detectedCategory] is not None and len(
                    objects[detectedCategory]):
                curGoodFrames += 1
    ...
    print(
        "PERF: Total time: {} Validation time {}, Homography avg"
        " {:.2f}ms Validation avg {:.2f}ms" .format(
            timeTotal,
            timeVal,
            (timeTotal - timeVal) * 1000 / (curFrame),
