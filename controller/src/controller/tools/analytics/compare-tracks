...
def build_argparser():
    ...
    parser.add_argument(
        "ground_truth",
        help="ground truth tracked data file",
        type=str)
    parser.add_argument(
        "predicted",
        help="predicted tracked data file",
        type=str)
...
def main():
    ...
    pred_data, pred_track_data, _ = json_helper.loadData(
        args.predicted, args.convert_json)
    gt_data, gt_track_data, gt_info = json_helper.loadData(args.ground_truth)
    pred_gid_map = metrics.getGIDLocs(pred_track_data)
    msoce, idc_error, std_velocity, std_max_velocity = getMetrics(
        gt_data, pred_data, pred_gid_map, gt_track_data)
