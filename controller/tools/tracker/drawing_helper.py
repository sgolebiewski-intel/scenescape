...
def labelObjects(objects, camDetect, frame, sensor, font, scale):
    ...
    for obj in camDetect:
        ...
        point = (
            np.array((bounds.x + bounds.width / 2, bounds.y + bounds.height / 2))
            / scale
        )
        label = "GID:%s" % (obj.gid)
        ...
        if obj.location[0].bounds.isPointWithin(pt):
            scl_line(
                frame,
                (int(bounds.x1), int(pt.y)),
                (int(bounds.x2), int(pt.y)),
                color,
                2,
            )
            scl_line(
                frame,
                (int(pt.x), int(bounds.y1)),
                (int(pt.x), int(bounds.y2)),
                color,
                2,
            )
