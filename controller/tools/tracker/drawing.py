...
# Wrappers to do drawing and automatically scale borders/font size/radius
# up to fit image size
...
def scl_line(image, start_point, end_point, color, thickness):
    ...
    return cv2.line(image, start_point, end_point,
                    color, int(thickness * scale))


def scl_rect(image, start_point, end_point, color, thickness):
    scale = image.shape[1] / _SCL_NORMAL
    return cv2.rectangle(
        image, start_point, end_point, color, int(
            thickness * scale))


def scl_circle(image, center, radius, color, thickness):
    scale = image.shape[1] / _SCL_NORMAL
    return cv2.circle(image, center, int(radius * scale),
                      color, int(thickness * scale))
...
def scl_text_size(image, label, point, point_rel, font, fsize, thickness):
    ...
    size, baseline = cv2.getTextSize(
        label, font, fsize * scale, int(thickness * scale))
...
def scl_text(image, label, point, point_rel, font, fsize, color, thickness):
    size, origin, scale = scl_text_size(
        image, label, point, point_rel, font, fsize, thickness)
    cv2.putText(image, label, origin, font, fsize *
                scale, color, int(thickness * scale))
    return size
