    ...
    if Path(file_path).suffix == '':
        with open(file_path, 'r') as f:
            ...
            if first_line == "#!/usr/bin/env python3" or first_line == "#!/bin/bash":
                subprocess.run(["make", f"add-licensing", "FILE=" +
                                file_path, "ADDITIONAL_LICENSING_ARGS=" +
                                "\"--style=python\""])
            else:
        ...
        subprocess.run(["make", "add-licensing", "FILE=" +
                        file_path, "ADDITIONAL_LICENSING_ARGS=" +
                        "\"--style=python\""])
    else:
        subprocess.run(["make", "add-licensing", "FILE=" + file_path])
...
REUSE_REPORT = subprocess.run(
    ["reuse", "lint", "-j"], capture_output=True, text=True).stdout
