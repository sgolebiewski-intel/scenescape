...
def main():
        ...
        if len(camDetect['objects']):
            ...
            for category in sorted_objs:
                ...
                topic = PubSub.formatTopic(
                    PubSub.DATA_CAMERA, camera_id=camDetect['id'])
    ...
    print("GEN: {}: Ran {:.3f} s, {} messages, {} detections published" .format(
        TestName, total_time, total_messages_published, total_detections_published))
    print(
        "GEN: Message Rate {:.3f}, Detection Rate {:.3f}".format(
            message_rate,
            object_rate))
