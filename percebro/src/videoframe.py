...
class VideoFrame:
    ...
    def initVirtual(self, frame, virtual):
        if virtual is not None and len(virtual):
            ...
            for bounds in virtual:
                vrect = Rectangle(
                    origin=Point(bounds[0], bounds[1]), size=(bounds[2], bounds[3])
                )
                frect = Rectangle(origin=Point(0, 0), size=tuple(frame.shape[1::-1]))
                irect = vrect.intersection(frect)
                if (
                    irect is None
                    or irect.size.width == 0
                    or irect.size.width < vrect.size.width
                    or irect.size.height < vrect.size.height
                ):
                    continue
    ...
    def adjustRectToFrame(self, frame, rect):
        ...
        if x1 < 0 or y1 < 0 or x2 >= img_w or y2 >= img_h:
            ...
            if y2 >= img_h:
                y2 = img_h - 1
            rect = Rectangle(origin=Point(x1, y1), opposite=Point(x2, y2))
        return rect
    ...
    def calculateBounds3D(self, frame, obj):
        ...
        world_vertices = Rectangle(
            origin=Point(obj["translation"]), size=tuple(obj["size"])
        )
        ...
        return self.adjustRectToFrame(
            frame, Rectangle(origin=Point(xmin, ymin), opposite=Point(xmax, ymax))
        )
    ...
    def cropBounds3DClosestFaceObjectRotate(self, frame, world_vertices, obj):
        ...
        if np.allclose(face_horizontal, [0, 0, 0], atol=0.5) or np.allclose(
            face_vertical, [0, 0, 0], atol=0.5
        ):
            log.warn(
        ...
        face_v1 = np.cross(face_horizontal, face_vertical)
        ...
        inv_face_transform = np.vstack(
            [
                np.hstack([inv_face_rotation, np.zeros(3).reshape((3, 1))]),
                np.array([0, 0, 0, 1]),
            ]
        )
        ...
        # original face
        ratio = np.sqrt(src_img_bb_area / dst_img_bb_area) * np.ptp(
            vertices_face_csys[:, :2], 0
        )
        fxy = np.divide(ratio, np.ptp(vertices_face_csys[:, :2], 0))[0]

        v_camera_matrix = np.array(
            [[fxy, 0, ratio[1] / 2], [0, fxy, ratio[0] / 2], [0, 0, 1]]
        )
        dst_img_pts = (
            cv2.projectPoints(
                vertices_face_csys, rvec, tvec, v_camera_matrix, np.zeros(4)
            )[0][:, 0, :]
            .round()
            .astype(int)
        )
        ...
        adjusted_frame = cv2.warpPerspective(
            frame, perspective_transform, ratio[::-1].astype(int)
        )
        ...
        if source is None:
            ...
            if len(self.input[source].data) != len(self.output[source].data):
                log.error(
                    "IO MISMATCH!",
                    source,
                    len(self.input[source].data),
                    len(self.output[source].data),
                )
                exit(1)
            virtual_len = 0
            ...
            for idx in range(len(self.input[source].data) - virtual_len):
                ...
                if (
                    isinstance(fdata, list)
                    and len(fdata) > 0
                    and isinstance(fdata[0], dict)
                ):
                    if "bounding_box" in fdata[0]:
                        bounds = (
                            Rectangle(fdata[0]["parent_bounding_box"])
                            if "parent_bounding_box" in fdata[0]
                            else Rectangle(fdata[0]["bounding_box"])
                        )
                        frames.extend(
                            self.cropBounds(self.input[source].data[idx], fdata)
                        )
                    else:
                        if self.disable_3d_rotation:
                            frames.extend(
                                self.cropBounds3D(self.input[source].data[idx], fdata)
                            )
                        else:
                            frames.extend(
    ...
    def annotatePlate(self, frame, bounds, text):
        ...
        lsize = cv2.getTextSize(
            text, cv2.FONT_HERSHEY_SIMPLEX, 1 * scale, int(5 * scale)
        )[0]
    ...
    def findParentTransform(self, parent_obj):
        ...
        parent_rect = Rectangle(
            origin=Point(parent_obj["translation"]),
    ...
    def annotateRotatedSubobject(self, frame, parent_obj, obj_bounds, cindex):
        ...
        bbox_img_pts = np.array(
            [
                [obj_bounds.x, obj_bounds.y],
    ...
    def annotateObjects(self, frame, objects):
        for obj in objects:
            ...
            if "distance" in obj:
                ...
                lsize = cv2.getTextSize(
                    label, cv2.FONT_HERSHEY_SIMPLEX, 1 * scale, 5 * scale
                )[0]
                cv2.putText(
                    frame,
                    label,
                    (int(bounds.x + 10), int(bounds.y + 10) + lsize[1]),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1 * scale,
                    (0, 0, 0),
                    5 * scale,
    ...
    def compute2DVertices(self, bbox3D, rotation):
        ...
        log.error("Division by zero: bbox", bbox3D, "image coords", pts_img)
        return None

    def annotate3DObject(
        self, frame, bbox3D, rotation, color=(66, 186, 150), thickness=2
    ):
    ...
    def updateFrameAverage(self):
        ...
        if oldFrameAvg is not None:
            ...
            if getattr(self, "begin", None) is not None:
                ...
                percent = (
                    abs(oldLatencyAvg - self.cam.latencyAvg) / self.cam.latencyAvg
                ) * 1000
                if (
                    self.cam.averageStable
                    and percent >= 0.6 * 100 / VideoFrame.AVG_FRAMES
                ):
                    self.cam.averageStable = False
