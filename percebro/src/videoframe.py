...
class VideoFrame:
    ...
    def __init__(self, cam, frame, virtual, id=None, depth=None,
        ...
        self.filtering_operations = {
            # Bottom finds the object with the max value using 'y' and 'height'
            # (bottom row in pixels)
            'bottom': {'axis': 'y', 'attribute': 'height', 'operator': max},
            # Top finds the object with the min value, using 'y' (top row in
            # pixels)
    ...
    def initVirtual(self, frame, virtual):
        if virtual is not None and len(virtual):
            ...
            for bounds in virtual:
                ...
                frect = Rectangle(origin=Point(
                    0, 0), size=tuple(frame.shape[1::-1]))
                ...
                if irect is None or irect.size.width == 0 \
                   or irect.size.width < vrect.size.width or irect.size.height < vrect.size.height:
                    continue
                crop = frame[int(vrect.y):int(vrect.y2),
                             int(vrect.x):int(vrect.x2)]
    ...
    def calculateBounds3D(self, frame, obj):
        ...
        world_vertices = Rectangle(
            origin=Point(
                obj['translation']), size=tuple(
                obj['size']))
        transformed_vertices = self.compute2DVertices(
            world_vertices, obj['rotation'])
        if transformed_vertices is None:
            return None
        xmin, ymin = np.min(transformed_vertices, axis=0)
        xmax, ymax = np.max(transformed_vertices, axis=0)
        return self.adjustRectToFrame(
            frame, Rectangle(
                origin=Point(
                    xmin, ymin), opposite=Point(
                    xmax, ymax)))
    ...
    def cropBounds3DRotate(self, frame, bounds):
        ...
        for idx, obj in enumerate(bounds):
            ...
            if out_frame is not None:
                ...
                self.perspective_transforms.append(
                    [world_vertices, pixel_perspective_transform])
            else:
    ...
    def cropBounds3DClosestFaceObjectRotate(self, frame, world_vertices, obj):
        ...
        vertices = self.getCuboidVertices(
            world_vertices, rotation=obj['rotation'])

        # Find the 4 closest vertices - this is the closest face.
        closest_vertices = np.array([vertices[i] for i in np.argsort(
            [np.linalg.norm(v) for v in vertices])], dtype=np.float32)[0:4]
        ...
        if np.allclose(face_horizontal, [0, 0, 0], atol=0.5) \
            ...
            log.warn(
                "Invalid bounds detected: horizontal vector",
                face_horizontal,
                "vertical vector",
                face_vertical,
                "source pixels",
        ...
        if inv_face_rotation_raw is None:
            log.warn(
                "Invalid Quat detected, xformed_vertices",
                xformed_vertices,
                "src_img_pts",
                src_img_pts,
                "norm_v1",
        ...
        v_cam_rotation = Rotation.from_euler(
            'XYZ', [0, np.pi, np.pi / 2]).as_matrix()
        ...
        # Get virtual camera focal length to maintain same pixel area as
        # original face
        ratio = np.sqrt(src_img_bb_area / dst_img_bb_area) * \
            np.ptp(vertices_face_csys[:, :2], 0)
    ...
    def prepareData(self, source, dest):
        if source is None:
            ...
            if self.output[source] is None or len(
                    self.output[source].data) == 0:
                self.output[dest] = None
                return None
            frames = []
            if len(self.input[source].data) != len(self.output[source].data):
                log.error(
                    "IO MISMATCH!", source, len(
                        self.input[source].data), len(
                        self.output[source].data))
                exit(1)
            virtual_len = 0
            if hasattr(
                    self.input[source],
                    'virtual') and self.input[source].virtual:
            ...
            for idx in range(len(self.input[source].data) - virtual_len):
                ...
                if isinstance(
                        fdata,
                        list) and len(fdata) > 0 and isinstance(
                        fdata[0],
                        dict):
                    if 'bounding_box' in fdata[0]:
                        bounds = Rectangle(
                            fdata[0]['parent_bounding_box']) if 'parent_bounding_box' in fdata[0] else Rectangle(
                            fdata[0]['bounding_box'])
                        frames.extend(
                            self.cropBounds(
                                self.input[source].data[idx],
                                fdata))
                    else:
                        if self.disable_3d_rotation:
                            frames.extend(
                                self.cropBounds3D(
                                    self.input[source].data[idx], fdata))
                        else:
                            frames.extend(
                                self.cropBounds3DRotate(
                                    self.input[source].data[idx], fdata))
    ...
    def filterResultsBoundingBox(self, fdata, axis, attribute, operator):
        ...
        # and offset when needed ('height' to get to the bottom-most pixel of
        # the detection).
        def detection_position(item):
            return item['bounding_box'][axis] + \
                item['bounding_box'].get(attribute, 0)

        # Find the detection of interest. operator max for bottom-most, min for
        # top-most.
    ...
    def mergeResults(self, toModel, fromModel):
        ...
        if hasattr(
                self.input[toModel],
                'virtual') and self.input[toModel].virtual:
        ...
        for tidx, tdata in enumerate(data_to):
            ...
            for tobj in tdata:
                ...
                if isinstance(
                        ...
                        list) and len(fdata) > 0 and isinstance(
                        fdata[0],
                        dict) and 'bounding_box' in fdata[0]:
                    ...
                    for oidx in range(len(fdata)):
                        ...
                        if not oe_bounds.is3D:
                            agnostic = self.cam.intrinsics.infer3DCoordsFrom2DDetection(
                                oe_bounds)
    ...
    def annotateHPE(self, frame, obj):
        ...
        for i in range(len(PoseEstimator.bodyPartKP)):
            ...
            cv2.circle(
                frame, pt1_int, 5, PoseEstimator.colors[i], -1, cv2.LINE_AA)
    ...
    def annotatePlate(self, frame, bounds, text):
        ...
        if not self.pointsInsideImage(
                frame, [[start_x, top_y], [end_x, bottom_y]]):
            log.warn(
    ...
    def findParentTransform(self, parent_obj):
        ...
        # 3d-object bounding box (closest face). We need to find which one was
        # used for this specific bounding box.
        parent_size = parent_obj['size']
        parent_rect = Rectangle(
            origin=Point(
                parent_obj['translation']), size=(
                parent_size[0], parent_size[1], parent_size[2]))

        for bbox_as_rect, pixel_transform in self.perspective_transforms:
            if bbox_as_rect.origin == parent_rect.origin and np.allclose(
                    bbox_as_rect.size.asNumpy[:2], parent_rect.size.asNumpy[:2]):
    ...
    def annotateRotatedSubobject(self, frame, parent_obj, obj_bounds, cindex):
        ...
        bbox_img_pts = np.array([[obj_bounds.x,
                                  obj_bounds.y],
                                 [obj_bounds.x + obj_bounds.width,
                                  obj_bounds.y],
                                 [obj_bounds.x,
                                  obj_bounds.y + obj_bounds.height],
                                 [obj_bounds.x + obj_bounds.width,
                                  obj_bounds.y + obj_bounds.height]])
        # Get the inverse to map the detection pixels back to parent_object
        # space:
        ...
        src_img_pts_mapped_back = cv2.perspectiveTransform(
            bbox_img_pts, inv_perspective_xform)
            ...
            if model in parent_obj:
                ...
                for obj in parent_obj[model]:
                    ...
                    if self.disable_3d_rotation:
                        ...
                        cv2.rectangle(
                            frame, *bounds.cv, VideoFrame.objColors[cindex], 4)
                    else:
                        # Get the pixel space bounds from inverse transform
                        obj_bbox_bounds = Rectangle(obj['bounding_box_px'])
                        bounds = self.annotateRotatedSubobject(
                            frame, parent_obj, obj_bbox_bounds, cindex)
                        if bounds is None:
                            continue

                    # Then try and find text (license plate) data in the
                    # sub-detection
    ...
    def annotateObjects(self, frame, objects):
        for obj in objects:
            ...
            if 'distance' in obj:
                ...
                lsize = cv2.getTextSize(
                    label, cv2.FONT_HERSHEY_SIMPLEX, 1 * scale, 5 * scale)[0]
                cv2.putText(frame,
                            label,
                            (int(bounds.x + 10),
                             int(bounds.y + 10) + lsize[1]),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            1 * scale,
                            (0,
                                0,
    ...
    def annotate3DObject(
        ...
        rvecs, tvecs = np.array([0, 0, 0], np.float64), np.array(
            [0, 0, 0], np.float64)
    ...
    def getCuboidVertices(self, bbox3D, rotation=None):
        ...
        vertices[0, [0, 1, 4, 5]], vertices[0, [
            2, 3, 6, 7]] = width / 2, -width / 2
        vertices[1, [0, 3, 4, 7]], vertices[1, [
            1, 2, 5, 6]] = height / 2, -height / 2
    ...
    def updateFrameAverage(self):
        ...
        if oldFrameAvg is not None:
            ...
            if getattr(self, 'begin', None) is not None:
                ...
                percent = (abs(oldLatencyAvg - self.cam.latencyAvg) /
                           self.cam.latencyAvg) * 1000
