...
class PostDecodeTimestampCapture:
    ...
    def processFrame(self, frame):
        ...
        if (now - self.last_calculated_fps_ts) > self.fps_calc_interval:
            self.fps = self.fps * self.fps_alpha + (1 - self.fps_alpha) * (
                self.frame_cnt / (now - self.last_calculated_fps_ts)
            )
            self.last_calculated_fps_ts = now
            self.frame_cnt = 0
        ...
        frame.add_message(
            json.dumps(
                {
                    "postdecode_timestamp": f"{
                        datetime.fromtimestamp(
                            now,
                            tz=timezone(TIMEZONE)).strftime(DATETIME_FORMAT)[
                            :-
                            3]}Z",
...
def computeObjBoundingBoxParams(
    pobj, fw, fh, x, y, w, h, xminnorm=None, yminnorm=None, xmaxnorm=None, ymaxnorm=None
):
...
class PostInferenceDataPublish:
    def __init__(
        self, cameraid, metadatagenpolicy="detectionPolicy", publish_image=False
    ):
    ...
    def annotateObjects(self, img):
        ...
        for otype, objects in self.frame_level_data["objects"].items():
            ...
            for obj in objects:
                topleft_cv = (
                    int(obj["bounding_box_px"]["x"]),
                    int(obj["bounding_box_px"]["y"]),
                )
                bottomright_cv = (
                    int(obj["bounding_box_px"]["x"] + obj["bounding_box_px"]["width"]),
    ...
    def annotateFPS(self, img, fpsval):
        # code snippet is taken from annotateFPS method in
        # percebro/videoframe.py
    ...
    def buildImgData(self, imgdatadict, gvaframe, annotate):
        imgdatadict.update(
            {"timestamp": self.frame_level_data["timestamp"], "id": self.cameraid}
        )
    ...
    def buildObjData(self, gvadata):
        ...
        self.frame_level_data.update(
            {
                ...
                "debug_processing_time": now
                - float(gvadata["timestamp_for_next_block"]),
        ...
        if "objects" in gvadata and len(gvadata["objects"]) > 0:
            framewidth, frameheight = (
                gvadata["resolution"]["width"],
                gvadata["resolution"]["height"],
            )
    ...
    def processFrame(self, frame):
        if self.client.is_connected():
            ...
            if self.is_publish_image:
                ...
                self.client.publish(
                    f"scenescape/image/camera/{self.cameraid}", json.dumps(imgdatadict)
                )
            ...
            if self.is_publish_calibration_image:
                ...
                self.client.publish(
                    f"scenescape/image/calibration/camera/{self.cameraid}",
                    json.dumps(imgdatadict),
                )
                self.is_publish_calibration_image = False

            self.client.publish(
                f"scenescape/data/camera/{self.cameraid}",
                json.dumps(self.frame_level_data),
            )
