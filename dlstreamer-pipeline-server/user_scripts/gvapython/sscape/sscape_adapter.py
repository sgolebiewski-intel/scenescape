...
class PostDecodeTimestampCapture:
    ...
    def processFrame(self, frame):
        ...
        if (now - self.last_calculated_fps_ts) > self.fps_calc_interval:
            self.fps = self.fps * self.fps_alpha + \
                (1 - self.fps_alpha) * (self.frame_cnt / (now - self.last_calculated_fps_ts))
            self.last_calculated_fps_ts = now
            self.frame_cnt = 0
        ...
        if self.ntpServer:
            ...
            if not self.lastTimeSync or now - self.lastTimeSync > 1000:
                response = self.ntpClient.request(
                    host=self.ntpServer, port=123)
...
def computeObjBoundingBoxParams(
    ...
    pobj.update({'center_of_mass': {'x': int(xmin + comw),
                                    'y': int(ymin + comh),
                                    'width': comw,
                                    'height': comh},
                 'bounding_box_px': {'x': x,
...
class PostInferenceDataPublish:
    ...
    def on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            ...
            print(
                f"Subscribed to topic: scenescape/cmd/camera/{self.cameraid}")
        else:
    ...
    def annotateObjects(self, img):
        objColors = ((0, 0, 255), (255, 128, 128),
                     (207, 83, 294), (31, 156, 238))
        for otype, objects in self.frame_level_data['objects'].items():
            if otype == "person":
                ...
                cindex = 2
            for obj in objects:
                topleft_cv = (int(obj['bounding_box_px']['x']), int(
                    obj['bounding_box_px']['y']))
    ...
    def processFrame(self, frame):
        if self.client.is_connected():
            ...
            if self.is_publish_calibration_image:
                ...
                self.client.publish(
                    f"scenescape/image/calibration/camera/{self.cameraid}", json.dumps(imgdatadict))
                self.is_publish_calibration_image = False

            self.client.publish(
                f"scenescape/data/camera/{self.cameraid}", json.dumps(self.frame_level_data))
